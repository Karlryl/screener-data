...
Node.js stock-screener pipeline. Pulls fundamentals + price data for ~15 300 global tickers daily, scores each ticker, classifies into Hypergrowth / Quality-Compounder / Turnaround discovery modes, and publishes a static HTML dashboard to GitHub Pages.

Engineer-to-engineer reference. No marketing copy. For chronological context see `audit-reports/`.

> **Partially superseded.** The `Architecture`, `Methods`, `Strategy modes`,
> and `Repository layout` sections below describe an older engine generation
> (`methods/`, `score-aggregator.js`, `engine-v7.3.js`, `generate-screener.js`,
> `tag28-tests.js`) that has been removed from the active tree. The current
> engine lives in `src/scoring/` (`run-screener.js` + `formulas/`); the
> current CI pipeline is `.github/workflows/daily-pull.yml` plus 6 auxiliary
> workflows. Binding rules: `CLAUDE.md`, `AGENTS.md`. Where a section below
> conflicts with the running code, the code wins — this README has not been
> fully rewritten for the new engine yet.
...
- **Universe:** ~15 300 tickers in `watchlist.json` as of HEAD. Auto-refreshed each run by `refresh-universe.js`.
- **Outputs:** `index.html` (dashboard) + `outputs/` tree, deployed to the `gh-pages` branch. `screener.html` / `modes-report.html` / `dashboard.html` / `diff-report.html` / `methods-report.html` were generated by the removed legacy generators (see banner above) and are not produced by the current pipeline.
- **Schedule:** `cron: '17 2 * * 2-6'` (Tue–Sat 02:17 UTC) plus `workflow_dispatch`. Main pipeline: `.github/workflows/daily-pull.yml`; 6 auxiliary workflows in the same directory (heartbeat, monthly plan-check, monthly SEC-XBRL, small-cap pull, TV reachability, weekly guard).
- **Wall-clock per run:** ~80 min for the last successful runs (Yahoo pull dominates; highest job `timeout-minutes: 200`).
...
│   ├── filter-snapshot-merge.js           # merge pre-stage: authorized-only filter
│   │                                      #   + NAV / quarantine registers (see Operational notes)
...
# 3. Historical prices + FX + earnings calendar
node pull-historical-prices.js --out prices --rate-limit 1500
node scripts/refresh-fx.js
node pull-earnings-dates.js

# 4. Score + classify (current engine)
node src/scoring/run-screener.js
node scripts/write-findash-export.js

# 5. Post-run analytics
node scripts/walk-forward-perf.js  # legacy/orphaned — not wired into CI, manual ad-hoc backtest only, not live scoring
...
- **Merge pre-stage / quarantine register.** `scripts/filter-snapshot-merge.js` runs as step *„F-12 Karteileichen-Filter"* in the `merge` job and copies only authorized snapshots from `snapshots-eingang/` to `snapshots/`. Two ticker registers gate that copy: `nav-holdings.json` (NAV holdings / closed-end funds) and `data-health/quarantine.json` — the latter for rows whose numbers **provably belong to a different company** (ENTSCHIED 37, PR #105; first and so far only case: `VMRK`, carrying AvalonBay's complete block). The bar is deliberately high, because a wrong entry deletes a real company from the board: proof of foreign numbers, a second independent pull with the same result, plus `beleg` + `pruefungBis` — an entry needs all five fields `ticker, grund, beleg, aufgenommen, pruefungBis` or the register fails to load. **Nothing is deleted** (Karl decision F-12): the input folder keeps every file, it just never reaches scoring. Unlike the three rename pre-stages (U2 BO/NS root twins, U3 Milan, T179 par-value), this one *discards* — there the identity is uncertain and the sealed dedup decides, here it is proven wrong.
- **Where the quarantine surfaces.** The line `QUARANTAENE: <n> Zeilen — <tickers> [Register: <m> Eintraege]` is printed to the step log on **every** run, including `n = 0` — a register that only speaks when it fires cannot be told apart from one that failed to load. On top of that, three loud failure modes as GitHub annotations: a register entry with no matching file in the input (`::warning::`, the exclusion is silently ineffective — delisted/renamed/typo), an entry past its re-review date (`::warning::`, see next bullet), and a quarantined file still sitting in the target directory from an earlier local run (`::error::` + hard stop; CI never hits this since `snapshots/` is git-ignored, local runs can). An unparseable or malformed register aborts the step instead of scoring poisoned rows (fail-closed).
- **Quarantine re-review (`pruefungBis`).** Every quarantine entry carries a mandatory re-review date. Once `pruefungBis` is older than the run date, each run warns `QUARANTAENE: <ticker> ist seit <datum> zur Wiedervorlage faellig …` — deliberately **not** a hard stop: the row is still poisoned, so the overdue entry keeps protecting correctly; what is overdue is the human decision to renew or retire it. This is what keeps the register a stop-gap rather than a graveyard. Guard: `tests/quarantaene.test.js` (checks the non-effect as sharply as the effect, and pins that discarding never touches the three rename pre-stages).
...
`PROJECT-STATUS.md` is SUPERSEDED — a frozen historical snapshot (Tag 239 era), not a current-state view. For chronological context, read `audit-reports/` in date order.
