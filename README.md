...
Node.js stock-screener pipeline. Pulls fundamentals + price data for ~11 100 global tickers daily, scores each ticker, classifies into Hypergrowth / Quality-Compounder / Turnaround discovery modes, and publishes a static HTML dashboard to GitHub Pages.
...
> current CI pipeline is `.github/workflows/daily-pull.yml` plus 5 auxiliary
...
- **Universe:** ~11 100 tickers in `watchlist.json` as of HEAD. Auto-refreshed each run by `refresh-universe.js`.
...
- **Schedule:** `cron: '17 2 * * 2-6'` (Tue–Sat 02:17 UTC) plus `workflow_dispatch`. Main pipeline: `.github/workflows/daily-pull.yml`; 5 auxiliary workflows in the same directory (heartbeat, monthly plan-check, monthly SEC-XBRL, TV reachability, weekly guard).
