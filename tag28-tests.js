...
// ─── Runner-Level ───
test('Runner: getMethods returns 22 methods', () => {
  const methods = Runner.getMethods();
  assertEq(methods.length, 22);
  const ids = methods.map(m => m.id).sort();
  assertEq(ids, ['above-200d-ma','aktienfinder-quality','capex-trend','drawdown-52w','ev-ebitda','fcf-yield','forward-pe','gross-margin-stability','high-proximity-52w','insider-ownership','margin-decay','multi-year-stability','net-debt-ebitda','peg','quarterly-rev-acceleration','revenue-growth-3y','roic','rule-of-40','sbc-revenue','sloan-ratio','volatility-annualized','working-capital-anomaly']);
});
