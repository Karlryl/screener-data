...
function loadAll(dir) {
  ...
  if (meta) {
    ...
    if (actualTickerCount !== meta.tickerCount) {
      const p = metaPath(dir);
      const err = new Error('price-history-store: ' + p + ' expects ' + meta.tickerCount
        + ' ticker(s), but the complete shard set contains ' + actualTickerCount
        + ' - partial/inconsistent store, refusing to load');
      err.metaPath = p;
