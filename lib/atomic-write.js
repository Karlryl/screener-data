...
// Tag 651 (audit F-CGPT-015, ADDITIV): dieses Modul schluckt an drei Stellen bewusst
// Fehler bzw. verzoegert still (Verzeichnis-fsync, Rename-Drossel, tmp-Aufraeumen).
// Das Verhalten bleibt exakt wie es war — aber es war bisher UNMESSBAR: ein dauerhaft
// fehlschlagender Verzeichnis-fsync loggte genau EINMAL pro Prozess und Verzeichnis
// (`_warnDirSyncOnce`), eine 380ms-Rename-Drossel gar nicht. Wer wissen will, ob die
// Schreibpfade sauber laufen, hatte keine Zahl. Ab jetzt zaehlen wir mit; niemand muss
// die Zaehler lesen, kein Aufrufer aendert sich.
// ponytail: nacktes Zaehler-Objekt statt Getter/Reset — Tests messen Deltas.
const atomicWriteStats = {
  dirFsyncFailures: 0,   // geschluckte Verzeichnis-fsync-Fehler (Rename-Metadaten evtl. nicht dauerhaft)
  renameRetries: 0,      // Windows-Drossel: Wiederholversuche nach EPERM/EBUSY/EACCES
  tmpCleanupFailures: 0, // geschluckte Aufraeum-Fehler der .tmp-Datei nach einem Schreibfehler
  lastDirFsyncError: null, // Meldung des letzten geschluckten Verzeichnis-fsync-Fehlers
};
...
function _sleepSync(ms) {
  ...
  //
  // audit F-A-2026-06-21 (A-lib-09): this sync sleep BLOCKS THE EVENT LOOP.
  // It is only reached from _renameWithRetry on Windows EPERM/EBUSY/EACCES,
  // and only for the duration of an actual contended rename. Worst case is the
  // full RENAME_RETRY_DELAYS_MS ladder = 10+20+50+100+200 = 380ms of stall on a
  // single write that stays locked across every retry. pull-yahoo writes one
  // snapshot per ticker, so in a pathological run where many writes hit a
  // sticky OneDrive/AV lock the stalls add up serially. We deliberately keep
  // the full ladder rather than trimming the [100,200] tail: the tail is the
  // last line of defence against the silent state-write-loss mode (F-SM-015)
  // when a OneDrive/AV handle is held longer than 50ms, and dropping it trades
  // a confirmed durability guarantee for an unproven latency win. The real fix
  // for the contention source (staging writes off the OneDrive-synced tree)
  // belongs in the caller, not in this module's sync contract.
  try {
...
function _renameWithRetry(tmp, targetPath) {
  ...
  try { fs.renameSync(tmp, targetPath); return; }
    ...
    for (const delay of RENAME_RETRY_DELAYS_MS) {
      atomicWriteStats.renameRetries++;
      _sleepSync(delay);
      try { fs.renameSync(tmp, targetPath); return; }
      catch (e2) {
...
function _fsyncParentDirBestEffort(targetPath) {
  ...
  try {
    ...
    // Verhalten unveraendert (kein Wurf) — nur jetzt zaehlbar statt einmal-geloggt.
    atomicWriteStats.dirFsyncFailures++;
    atomicWriteStats.lastDirFsyncError = (e && e.message) || String(e);
...
/**
 * Write a sibling temporary file, fsync it, and rename it over the target.
 * @param {string} targetPath - Destination file path.
 * @param {string|Buffer|ArrayBufferView} data - Bytes or text to persist.
 * @param {string|Object} [options] - Options passed to fs.writeFileSync.
 * @returns {void} Completes after replacement and best-effort directory fsync.
 * @throws {Error} File creation, writing, fsync, or replacement failed.
 */
function writeFileAtomic(targetPath, data, options) {
  ...
  try {
    ...
    // ENOENT ist kein Aufraeum-Fehler: die tmp-Datei kann es nie gegeben haben
    // (openSync fehlgeschlagen) oder der rename hat sie bereits weggezogen.
    try { fs.unlinkSync(tmp); } catch (e2) { if (e2 && e2.code !== 'ENOENT') atomicWriteStats.tmpCleanupFailures++; }
    throw err;
  }
}

// audit F-A-2026-06-21 (A-lib-08): a broken ratio/division upstream can yield
// NaN/Infinity/-Infinity; JSON.stringify silently rewrites all three to the
// literal `null`, so a corrupt number is persisted as a benign-looking missing
// value and later read back without any signal that the computation failed.
// Non-finite numbers are rejected by default: JSON.stringify would otherwise
// silently turn them into benign-looking nulls. A caller which deliberately
// accepts that lossy JSON behaviour must opt out explicitly with
// `{ assertFinite: false }`; ordinary null values are unaffected.
function _finiteAssertReplacer(userReplacer) {
  // prevents NaN/Infinity from a broken ratio being silently persisted as null
  // and later read back as benign-missing.
  return function (key, val) {
    // Apply the caller's replacer first so we validate the value that will
    // actually be serialised (a replacer may legitimately map values away).
    const out = typeof userReplacer === 'function'
      ? userReplacer.call(this, key, val)
      : val;
    if (typeof out === 'number' && !Number.isFinite(out)) {
      throw new Error(
        '[atomic-write] refusing to persist non-finite number (' + String(out) +
        ') at key ' + JSON.stringify(key) +
        ' — assertFinite is on; a NaN/Infinity here would be silently written as null');
    }
    return out;
  };
}
...
/**
 * Serialize JSON and replace its destination through the atomic writer.
 * @param {string} targetPath - Destination JSON path.
 * @param {Object|Array<unknown>|string|number|boolean|null} value - JSON-serializable value.
 * @param {{indent?: number|string, replacer?: Function|Array<string|number>, assertFinite?: boolean}} [opts] - Formatting, serialization, and finite-number checks.
 * @returns {void} Writes the serialized value.
 * @throws {Error} Serialization, a finite-number check, or the atomic write failed.
 */
function writeJsonAtomic(targetPath, value, opts) {
  opts = opts || {};
  const indent = (opts.indent != null) ? opts.indent : 2;
  const replacer = opts.assertFinite !== false
    ? _finiteAssertReplacer(opts.replacer)
    : (opts.replacer || null);
  const payload = JSON.stringify(value, replacer, indent);
  writeFileAtomic(targetPath, payload, { encoding: 'utf8' });
}

module.exports = { writeFileAtomic, writeJsonAtomic, atomicWriteStats };
