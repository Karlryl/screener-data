...
// Opt-in `assertFinite` makes the corrupt write fail LOUDLY at write time
// instead of poisoning the state file. Default stays off so existing callers
// (which legitimately serialise nulls) are unaffected.
...
function writeJsonAtomic(targetPath, value, opts) {
  ...
  const replacer = opts.assertFinite
