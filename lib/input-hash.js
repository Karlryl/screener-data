 ...
 * des GQS-00-Siegels.
 *
 * ============================ BEKANNTE LUECKE, NICHT BENUTZEN OHNE SIE ZU KENNEN ===========
 * Der KOHORTE-Teil ist UNVOLLSTAENDIG. Gerichtsverfahren 23.08. (Fall Fussabdruck-Vertrag,
 * DENIED) hat es am Artefakt nachgewiesen: `src/scoring/` liest DREIZEHN `meta.*`-Felder, der
 * Hash unten deckt DREI davon. Reproduzierter Fall: SOFI, `meta.industry` von "Credit Services"
 * auf "Software - Infrastructure" geaendert -> das Ergebnis springt von `exclude/` auf
 * `route/financials/50`, waehrend serienHash, kohorteHash UND gesamt byte-identisch bleiben.
 * `router.js` liest `meta.industry` an vier Stellen (:154, :190, :216, :265).
 * Ebenfalls ungedeckt: name, country, region, tradingCurrency, reportingCurrencyOriginal,
 * exchangeName, exchange, _annualCurrencyLeakSuspect - dazu die Bloecke metrics, external,
 * identifier.
 *
 * URSACHE, und sie ist die Lektion: die SERIEN-Liste wird aus dem FIELD_REGISTRY abgeleitet
 * und ist deshalb vollstaendig. Die KOHORTE-Liste wurde von Hand geschrieben - und war es
 * nicht. Die Heilung ist nicht "mehr Felder eintragen", sondern die Liste ebenfalls aus dem
 * Scoring-Code ABZULEITEN und per Waechter gegen diese Ableitung zu pinnen.
 *
 * Bis dahin gilt: der `serienHash` traegt, der `kohorteHash` traegt NUR fuer Marktwert,
 * Sektor, IPO-Jahr und Erst-Handelstag. Wer `gesamt` als "Eingang unveraendert" liest,
