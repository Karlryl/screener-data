 ...
 * @param {Buffer} schwanz - Dateiende des ZIPs mit moeglichem EOCD-Record.
 * @returns {number} Offset des EOCD-Signaturtreffers oder -1.
 */
 ...
 * @param {Buffer} cd - Bytes des zentralen Verzeichnisses.
 * @param {number} [anzahlErwartet] - Optional erwartete Anzahl der Eintraege.
 * @returns {Array<{name: string, methode: number, compressedSize: number, uncompressedSize: number, lfhOffset: number}>} Gelesene ZIP-Eintraege.
 * @throws {Error} Verzeichnisstruktur oder erwartete Eintragsanzahl passen nicht.
 */
...
/**
 * Reserviert den Leseblock fuer lokalen ZIP-Header, Dateiname und komprimierte Nutzdaten.
 * @param {{compressedSize: number}} eintrag - Eintrag aus dem zentralen ZIP-Verzeichnis.
 * @returns {number} Komprimierte Laenge plus 30 Header- und 4096 Reservebytes.
 */
