...
const { classify, resolveWindow, PRICE_MAX_STALE_DAYS } = require('./forward-returns.js');
const wf = require('../scripts/walk-forward-perf.js');

// Grenzen-Audit C-5 (03.08.2026): PRICE_MAX_STALE_DAYS stand ZWEIMAL im Repo — im Original
// scripts/walk-forward-perf.js und als handgepflegte Kopie hier daneben ("not exported by
// walk-forward-perf.js, so we define it here"). Beide messen dieselbe Sache: ab wann ein
// Einstiegskurs zu alt ist, um als Kurs zu gelten. Wer eine der Kopien anfasst, aendert die
// Messung nur zur Haelfte — und beide Haelften stehen in derselben Auswertung nebeneinander.
// Der Waechter prueft den WERT beider Ausgaenge gegeneinander, nicht die Schreibweise.
test('C-5: die Stale-Schwelle hat genau EINE Quelle (forward-returns == walk-forward-perf)', () => {
  assert.equal(typeof wf.PRICE_MAX_STALE_DAYS, 'number', 'das Original muss die Schwelle exportieren');
  assert.equal(PRICE_MAX_STALE_DAYS, wf.PRICE_MAX_STALE_DAYS,
    'zwei verschiedene Stale-Schwellen in derselben Auswertung');
});
...
test('resolveWindow pins the canonical benchmark window and accepts an ISO timestamp', () => {
  const prices = new Map();
  for (let offset = 0; prices.size < 45; offset++) {
    const date = new Date(Date.UTC(2025, 11, 1 + offset));
    if (date.getUTCDay() !== 0 && date.getUTCDay() !== 6) {
      prices.set(date.toISOString().slice(0, 10), 100 + prices.size);
    }
  }
  assert.deepEqual(resolveWindow({ SPY: prices }, '2026-01-05T18:30:00Z', 14), {
    insufficient: false,
    entryDate: '2026-01-05',
    exitDate: '2026-01-19',
    benchmarkTicker: 'SPY',
    horizonActualDays: 14,
  });
});

test('resolveWindow reports insufficient when benchmarks are missing or too short', () => {
  assert.deepEqual(resolveWindow({}, '2026-01-05', 14), { insufficient: true });
  const short = mapOf([['2026-01-05', 100], ['2026-01-19', 110]]);
  assert.deepEqual(resolveWindow({ SPY: short }, '2026-01-05', 14), { insufficient: true });
});

test('resolveWindow rejects old prices even when the benchmark has enough observations', () => {
  const old = new Map();
  for (let day = 1; day <= 45; day++) {
    old.set(new Date(Date.UTC(2024, 0, day)).toISOString().slice(0, 10), 100 + day);
  }
  assert.deepEqual(resolveWindow({ SPY: old }, '2026-01-05', 14), { insufficient: true });
});
