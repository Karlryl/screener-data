 ...
 * @param {number} seed - Ganzzahliger Startzustand des Zufallsgenerators.
 * @returns {function(): number} Reproduzierbarer Generator mit Werten in [0, 1).
 */
function rng(seed) {
  let a = seed >>> 0;
 ...
 * @param {number} p - Wahrscheinlichkeit im offenen Intervall (0, 1).
 * @returns {number} Approximiertes Standardnormalquantil; NaN ausserhalb des Intervalls.
 */
function probit(p) {
 ...
 * @param {Array<{state: string, outcome: string}>} entries - Aufgeloeste Eintraege beider Zustandsgruppen.
 * @returns {Object} Kontrast, Gruppenanteile und Gruppengroessen.
 */
 ...
 * @param {Array<{state: string, outcome: string}>} entries - Eintraege in der Reihenfolge der Ausgabe.
 * @param {{pC: number, pW: number, nC: number, nW: number}} pe - Gruppenkennzahlen des Punktschaetzers.
 * @returns {number[]} Einflussbeitraege je Eintritt.
 */
 ...
 * @param {Array<{block: string|number}>} entries - Eintraege mit Blockkennung.
 * @param {number[]} infl - Positionsgleiche Einflussbeitraege.
 * @returns {number[]} Summierte Einflussbeitraege je Block.
 */
 ...
 * @param {Array<{ticker: string, arm: number, block: string|number}>} entries - Eintraege mit Firmen-, Arm- und Blockkennung.
 * @returns { {ok: boolean, violations: Array<Object>} } Befund ueber Firmen, die mehrere Bloecke belegen.
 */
 ...
 * @param {{entries: Array<Object>, alphaStar: number, beta: number, B?: number, seed?: number}} opts - Aufgeloeste Eintraege, Fehlerniveaus und Bootstrap-Parameter.
 * @returns {Object} Lesbarkeitsbefund, Punktschaetzer und bei ausreichender Besetzung Bootstrap-Grenzen.
 * @throws {Error} Eine berechnete Ergebniskennzahl ist nicht endlich.
 */
 ...
 * @param {{armsRead: number[], sampleBars63: number[], sampleBars126: number[], thresholds?: Object, alphaStar?: number, scoreboard?: Object}} opts - Registrierte Arme, Kollapsstichproben und optionaler Test-Seam.
 * @returns {Object} Bestandene Kollaps- und Armzahlpruefung.
 * @throws {Error} Arme kollabieren oder Armzahl beziehungsweise alpha* widersprechen der Registrierung.
 */
