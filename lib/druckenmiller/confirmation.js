...
const SIGMA_WINDOW = 63;   // Barrieren-sigma (Datei B: barrier.sigmaWindowBars)
 ...
 * @param {Array<{date: string, close: number}>} series - Moeglicherweise unsortierte Kursreihe.
 * @param {string} sessionDate - Sitzungstag mit zwingend vorhandenem Balken.
 * @returns {number[]|null} Positive endliche Schlusskurse bis zum Sitzungstag oder null.
 */
 ...
 * @param {number[]} closes - Chronologisch geordnete Schlusskurse.
 * @param {number} fenster - Zahl der Tagesrenditen im Fenster.
 * @returns {number|null} Stichprobenvolatilitaet der Log-Renditen oder null.
 */
 ...
 * @param {Array<{date: string, close: number}>|null} series - Kursreihe fuer den Ticker.
 * @param {string} sessionDate - Auszuwertender Sitzungstag.
 * @returns { {m1: number|null, m2: number|null, vol252: number|null, sigma63: number|null, bars: number, close: number}|null } Kennzahlen oder null ohne Tagesbalken.
 */
function metricsFor(series, sessionDate) {
  ...
  // sigma63 wird HIER mitgerechnet, weil die Serie nur in diesem Durchgang im Speicher
  // liegt (der Logger laedt 32 Shards nacheinander). Ein zweiter Preis-Durchgang nur fuer
  // die Barrieren-Breite waere der teuerste Weg zur selben Zahl.
  return { m1, m2, vol252: vol, sigma63: dailyLogVol(closes, SIGMA_WINDOW), bars: n, close: closes[n - 1] };
}

/** Die Zeilen-Beilage des Loggers: genau die Chunk-2-Felder, nichts sonst.
 * @param {Array<{date: string, close: number}>|null} series - Kursreihe fuer den Ticker.
 * @param {string} sessionDate - Auszuwertender Sitzungstag.
 * @returns { {m1: number|null, m2: number|null, sigma63: number|null} } Zusatzfelder mit sichtbaren Datenluecken.
 */
function rowExtra(series, sessionDate) {
  const m = metricsFor(series, sessionDate);
  if (!m) return { m1: null, m2: null, sigma63: null };
  return { m1: m.m1, m2: m.m2, sigma63: m.sigma63 };
}
 ...
 * @param {number[]} values - Querschnittswerte; nur endliche Werte zaehlen.
 * @param {number} [minN] - Mindestbesetzung; Standard 30.
 * @returns { {q33: number, q67: number, n: number}|null } Type-7-Terzilgrenzen oder null unter Mindest-N.
 */
 ...
 * @param {number|null} m1 - Erste Tickerkennzahl.
 * @param {number|null} m2 - Zweite Tickerkennzahl.
 * @param {{q33: number, q67: number}|null} cutsM1 - Terzilgrenzen der ersten Kennzahl.
 * @param {{q33: number, q67: number}|null} cutsM2 - Terzilgrenzen der zweiten Kennzahl.
 * @returns { {state: string|null, reason: string|null, degenerate: boolean} } Zustand mit Grund und Degenerationshinweis.
 */
 ...
 * @param {Array<string|null>} states - Zustaende des Tickerquerschnitts.
 * @returns { {confirmsShare: number|null, raw: boolean, reason: string|null} } Trennungsbefund samt CONFIRMS-Anteil.
 */
...
/**
 * Die Zustands-Tabelle einer Sitzung aus den ROH-ZEILEN des Loggers (die tragen m1/m2/
 * sigma63 schon, siehe rowExtra). Nenner sind NUR die Zeilen, die im Universum sind UND am
 * Sitzungstag einen Balken haben (A3/[REV3-3]) — alle anderen bekommen ausdruecklich
 * `state: null`, nie ein stilles NEUTRAL.
 * @param {Array<Object>} rows - Tickerzeilen mit inUniverse, atSession, m1 und m2.
 * @param {number} [minCutN] - Mindestbesetzung der Terzil-Schnitte.
 * @returns {Object} Zustandszeilen, Schnittwerte, Ausschlusszaehler und Trennungsgate.
 */
function sessionStates(rows, minCutN) {
  const kandidaten = [];
  for (const r of rows) {
    if (!r || !r.inUniverse || !r.atSession) {
      kandidaten.push({
        ticker: r && r.ticker, m1: null, m2: null, sigma63: null, close: null,
        state: null, reason: 'not-in-universe',
      });
      continue;
    }
    kandidaten.push({
      ticker: r.ticker,
      m1: Number.isFinite(r.m1) ? r.m1 : null,
      m2: Number.isFinite(r.m2) ? r.m2 : null,
      sigma63: Number.isFinite(r.sigma63) ? r.sigma63 : null,
      close: Number.isFinite(r.close) ? r.close : null,
...
module.exports = {
  M1_LOOKBACK, M1_LAG, VOL_WINDOW, HIGH_WINDOW, SIGMA_WINDOW, SEPARATION_MAX, SEPARATION_MIN, STATES,
  closesThrough, dailyLogVol, metricsFor, rowExtra, terciles, stateOf, separationGate, sessionStates,
};
