 ...
 * @param {Array<{date: string, close: number}>} series - Kursreihe des Tickers; unsortierte Werte sind erlaubt.
 * @param {string} sessionDate - Auszuwertender Sitzungstag.
 * @returns {Object|null} Kurs, Historienlaenge, Mittelwerte und Rendite; null ohne Sitzungstagsbalken.
 */
 ...
 * @param {Array<Object>} rows - Tickerzeilen mit close und dem Vergleichsfeld.
 * @param {string} feld - Name des Vergleichsfelds.
 * @returns {number|null} Anteil oberhalb des Feldes unter abgedeckten Zeilen; null ohne Abdeckung.
 */
 ...
 * @param {Array<{close: number, high252: number, low252: number}>} rows - Tickerzeilen mit Jahresband.
 * @param {number} band - Relativer Abstand vom Hoch beziehungsweise Tief.
 * @returns {number|null} Hochnaehe-Anteil minus Tiefnaehe-Anteil oder null ohne Abdeckung.
 */
 ...
 * @param {Array<{sector: string, ret63: number, marketCap: number}>} rows - Tickerzeilen mit Sektor, Rendite und Gewicht.
 * @returns {Object} Gleich- und kapitalgewichteter Spread sowie Korb- und Zuordnungszaehler.
 */
 ...
 * @param {Array<{industry: string, sector: string, ret63: number}>} rows - Tickerzeilen mit Branchen- und Sektorzuordnung.
 * @returns { {value: number|null, n: number} } Korbspread oder null unter Mindestbesetzung.
 */
 ...
 * @param {Array<{netRevision30: number|null}>} rows - Netto-Revisionen der Universumsmitglieder.
 * @returns { {value: number|null, coverage: number|null} } Anteil positiver Revisionen und Datenabdeckung.
 */
 ...
 * @param {Array<{sector: string, ret63: number}>} rows - Tickerzeilen mit Sektor und 63-Tage-Rendite.
 * @param {number|null} spyRet63 - Vergleichsrendite des SPY.
 * @returns {Array<{sector: string, rs63: number, rank: number}>} Absteigend gerangte Sektoren; leer ohne SPY.
 */
 ...
 * @param {Array<{sector: string, rank: number}>} jetzt - Aktuelle Sektorraenge.
 * @param {Array<{sector: string, rank: number}>|null} vorher - Raenge der vorherigen Sitzung.
 * @returns {number|null} Spearman-Korrelation gemeinsamer Sektoren oder null.
 */
 ...
 * @param {Array<{ret63: number, marketCap: number}>} rows - Ticker-Renditen und Kapitalgewichte.
 * @returns { {capMinusEqual: number|null, topDecileShare: number|null, nWinners: number} } Gewichtungsspread und messbarer Gewinnanteil des obersten Dezils.
 */
 ...
 * Aggregaten (A3).
 * @param {Map<string, Object>} candidates - Kandidaten mit Sektor-, Branchen- und Revisionsdaten.
 * @param {Map<string, Array<{date: string, close: number}>>} seriesByTicker - Lokale Kursreihen nach Ticker.
 * @param {string} sessionDate - Auszuwertender Sitzungstag.
 * @param {function(Array<Object>|null, string): Object} [extra] - Optionaler Hook fuer Zusatzfelder derselben Sitzung.
 * @returns {Array<Object>} Eine Rohzeile je Kandidat, einschliesslich sichtbarer Serien- und Frischeluecken.
 */
function perTickerRows(candidates, seriesByTicker, sessionDate, extra) {
  // `extra` ist der Haken fuer Chunk 2: der Logger haengt m1/m2/sigma63 an, die NUR in
  // diesem Durchgang billig sind (die Serie liegt hier im Speicher). internals.js selbst
  // kennt diese Kennzahlen nicht — es haengt an, was es bekommt, und nichts sonst.
  const beilage = typeof extra === 'function' ? extra : null;
  const out = [];
  for (const [ticker, k] of candidates) {
    ...
    if (!s || !s.length) {
      out.push(Object.assign(beilage ? beilage(null, sessionDate) : {}, {
    ...
    out.push(Object.assign(beilage ? beilage(s, sessionDate) : {}, {
      ticker,
 ...
 * @param {number[]} werte - Vergleichsverteilung; nicht endliche Werte werden verworfen.
 * @param {number} wert - Einzuordnender Wert.
 * @returns {number|null} Mittelrang in [0, 1] oder null ohne brauchbare Daten.
 */
 ...
 * @param {Array<Object>|null} history - Bisherige Ledgerzeilen mit Achsen und Ausschlussmarkierungen.
 * @param {Object<string, number|null>} jetzt - Aktuelle Werte von l1, l2, l3 und l4ew.
 * @returns {number|null} Mittlerer expandierender Achsenrang; null vor 250 geeigneten Live-Tagen.
 */
 ...
 * @param {Array<{lastBarDate: string|null}>} rawRows - Letzter Balkentag jedes Kandidaten.
 * @returns {string|null} Haeufigster Tag, bei Gleichstand der neuere; null ohne Datumsangaben.
 */
 ...
 * @param {{date: string, rawRows: Array<Object>, backfilled?: boolean, spyState?: string, spyRet63?: number, iwmRet63?: number, prevRow?: Object, history?: Array<Object>, snapshotUnreadable?: number, now?: Date}} options - Sitzungsdaten, Rohzeilen, Vergleichsrenditen und bisherige Historie.
 * @returns {Object} Vollstaendige Ledgerzeile mit Universum, Datenqualitaet und L1-L8-Achsen.
 */
