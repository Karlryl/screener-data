...
const LEDGER_ROW_FIELDS = [
  ...
  'lowFreshness', 'nAtSession', 'excludedNoBar',
...
function buildRow({ date, rawRows, backfilled, spyState, spyRet63, iwmRet63, prevRow, history, universeHash, now }) {
  ...
  const row = {
    ...
    // FRISCHE nur fuer LIVE-Zeilen (Gericht, Wiederaufnahme 14.09.2026). Bei einer
    // rueckgerechneten Zeile beschreibt der modale Balken-Tag den Store von HEUTE, nicht
    // die Lage an jenem Handelstag — er waere fuer alle Backfill-Zeilen derselbe Wert und
    // wuerde eine Frische behaupten, die niemand gemessen hat. Darum null und vom Tor
    // ausgenommen; die Teilnahme an der Sitzung steht weiterhin in nAtSession/excludedNoBar.
    barDateMode: backfilled === true ? null : mode,
    mixedBarDateShare: backfilled === true ? null : (rows.length ? abweichend / rows.length : null),
    freshShare: backfilled === true ? null : (rows.length ? 1 - abweichend / rows.length : null),
    nExcludedStale: backfilled === true ? null : abweichend,
    lowFreshness: backfilled === true ? false : (rows.length ? (1 - abweichend / rows.length) < FRESH_MIN : true),
