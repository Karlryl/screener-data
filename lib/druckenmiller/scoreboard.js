...
/**
 * Bildet die stabile Entry-Kennung aus Ticker, Eintrittstag und Arm.
 * @param {{ticker: string, date: string, arm: number}} e - Scoreboard-Eintritt.
 * @returns {string} Mit senkrechten Strichen verbundene Entry-Kennung.
 */
 ...
 * @param {number[]} closes - Chronologische Schlusskurse bis zum Eintritt.
 * @returns {number|null} Positive 63-Tage-Log-Volatilitaet oder null.
 */
 ...
 * @param {{backfilled?: boolean, lowFreshness?: boolean, highChurn?: boolean, raw?: boolean}|null} session - Sitzungsbefund mit Ausschlussmarkierungen.
 * @returns { {eligible: boolean, reason: string|null} } Eintrittszulassung mit erstem Ausschlussgrund.
 */
 ...
 * @param {Map<string, string>} lastStates - Letzter beobachteter Zustand je Ticker.
 * @param {Array<{ticker: string, state: string|null, m1?: number, m2?: number}>} stated - Aktuelle beobachtete Zustaende.
 * @returns {Array<Object>} Beobachtete Zustandswechsel einschliesslich Erstbeobachtungen.
 */
 ...
 * @param {number|null|undefined} lastEntryIndex - Letzter Eintrittsindex desselben Tickers und Arms.
 * @param {number} currentIndex - Aktueller Sitzungsindex.
 * @param {number} arm - Vorgeschriebene Zahl wartender Sitzungen.
 * @returns {boolean} Ob kein alter Eintritt besteht oder dessen Abkuehlzeit verstrichen ist.
 */
 ...
 * @param {{session: Object, stated: Array<Object>, lastStates: Map<string, string>, lastEntryIndex: Map<string, number>, sessionIndex: number, warmup?: boolean}} opts - Sitzung, Zustandshistorie und Eintrittszaehler.
 * @returns {Object} Neue Eintraege, Wechselzaehler und moeglicher Ausschlussgrund.
 */
 ...
 * @param {{arm: number, k: number, sigma63: number, entryClose: number}} entry - Eintritt mit festgehaltenen Barrierenparametern.
 * @param {Array<{date: string, close: number}>} forward - Schlusskurse nach dem Eintritt in Reihenfolge.
 * @param {string} scaling - Explizit registrierte Skalierung daily oder horizon.
 * @returns {Object} UP, DOWN, CENSORED oder noch offener Verlauf mit Barrieren und Balkenzahl.
 * @throws {Error} Die angegebene Skalierung fehlt oder ist nicht implementiert.
 */
 ...
 * @param {Object} entry - Urspruenglicher Eintritt mit Ticker, Datum und Arm.
 * @param {Object} res - Erstpassage-Ergebnis mit Datum, Outcome und Barrieren.
 * @param {string} generatedAt - Zeitpunkt der Erzeugung mit Zeitzone.
 * @returns {Object} Separate append-only Aufloesungszeile.
 */
 ...
 * @param {string[]} entryDates - Eintrittstage.
 * @param {string[]} sessions - Geordnetes Sitzungstagsraster.
 * @param {number} blockBars - Zahl der Sitzungen je nicht ueberlappendem Block.
 * @returns {number} Zahl belegter nicht ueberlappender Bloecke.
 */
 ...
 * @param {number} L - Untere Effektgrenze in Prozentpunkten.
 * @returns { {id: number|null, text: string} } Registriertes Etikett oder noch nicht lesbar.
 */
function labelFor(L) {
 ...
 * @param {number} armsRead - Zahl formal mitgelesener Arme.
 * @returns {number} Registriertes ein- oder zweiarmiges alpha*.
 */
 ...
 * @param {{readId: string, blocksReached: boolean, L: number, mde: number}} opts - Lesung und ihre Entscheidungskennzahlen.
 * @returns { {branch: string|null, retire: boolean, reason: string} } Registrierter Stilllegungszweig.
 */
 ...
 * @param {string} iso - Ausgangstag YYYY-MM-DD.
 * @param {number} m - Monatsabstand vom Ausgangsmonat.
 * @returns {string} Letzter Kalendertag des Zielmonats.
 */
 ...
 * @param {string} t0 - Hash-Datum der Registrierung.
 * @param {string|null} r1Ready - Erster Tag mit erfuellten R1-Bedingungen.
 * @returns { {T0: string, R1: string|null, R2: string, R3: string} } Kalender der zugelassenen Lesungen.
 */
 ...
 * @param {{R1: string|null, R2: string, R3: string}} schedule - Registrierter Leseplan.
 * @param {string|null} readId - Zuletzt abgeschlossene Lesung.
 * @returns {string|null} Naechster Lesetag; nach R3 null.
 */
 ...
 * @param {Object|null} lastRead - Eingefrorener letzter Lesungsdatensatz.
 * @param {Object|null} counters - Aktuelle Tageszaehler.
 * @param {Object} schedule - Kalender der Lesungen.
 * @returns {Object} Getrennte eingefrorene und laufende Anzeigefelder samt Lesungsstempel.
 */
 ...
 * @param {Array<Object>} sessionRows - Chronologische Sitzungszeilen mit Eintraegen, Aufloesungen und Zustandswechseln.
 * @returns {Map<string, string>} Letzter beobachteter Zustand je Ticker.
 */
 ...
 * @param {Array<Object>} sessionRows - Chronologische Sitzungszeilen mit Eintraegen, Aufloesungen und Zustandswechseln.
 * @returns {Map<string, number>} Letzter Sitzungsindex je Ticker und Arm.
 */
 ...
 * @param {Array<Object>} sessionRows - Chronologische Sitzungszeilen mit Eintraegen, Aufloesungen und Zustandswechseln.
 * @returns {Array<Object>} Eintraege ohne passende Aufloesung.
 */
 ...
 * @param {Array<Object>} sessionRows - Chronologische Sitzungszeilen mit Eintraegen, Aufloesungen und Zustandswechseln.
 * @returns {number} Zahl explizit nicht rueckgerechneter Sitzungen.
 */
 ...
 * @param {string} registered - In Datei B registrierte Skalierung.
 * @returns { {scaling: string, factorAt63: number, factorAt126: number} } Implementierter Zweig und beide Faktoren.
 * @throws {Error} Der registrierte Zweig ist nicht implementiert.
 */
 ...
 * @param {number[]} a - Erste Stichprobe.
 * @param {number[]} b - Zweite Stichprobe.
 * @returns {Object} KS-Abstand und skalierter Abstand; null bei leerer Stichprobe.
 */
...
/**
 * Berechnet den Median einer Zahlenreihe auf einer sortierten Kopie.
 * @param {number[]} xs - Numerische Ausgangswerte.
 * @returns {number|null} Median oder null fuer eine leere Reihe.
 */
const medianOf = (xs) => {
 ...
 * @param {number[]} bars63 - Aufloesungszeiten des 63-Arms.
 * @param {number[]} bars126 - Aufloesungszeiten des 126-Arms.
 * @param {{ksScaledMin?: number, medianRatioMin?: number}} [thresholds] - Registrierte Mindestunterschiede.
 * @returns {Object} Kollapsbefund mit KS-Test, Medianen, Schwellen und Sperrgrund.
 */
 ...
 * @param {number[]} arms - Formal getrennte Arme der Lesung.
 * @returns { {arms: number, alphaStar: number, ok: boolean} } Armzahl und dazu passende registrierte Korrektur.
 */
