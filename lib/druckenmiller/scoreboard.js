 ...
 *
 * ANHAENGIG-MACHEN DER SKALIERUNG (offen, Rats-Sache): die Spezifikation sagt
 * "k · sigma mit k = 1 (sigma = 63-Tage-Tagesvol bei Eintritt)". Woertlich gelesen ist die
 * Barriere EINE Tagesvol breit; auf echten Daten (687 Ticker, Shard 0, gemessen
 * 2026-09-19) liegt die Erstpassage dann im Median bei 3 Balken, 19 % loesen am ersten
 * Balken auf, 95 % innerhalb von 63 — der Unterschied zwischen dem 63- und dem
 * 126-Balken-Arm verschwindet, und mit ihm die Abkuehlzeit und die Blockrechnung. Mit der
 * horizont-skalierten Lesung (sigma * sqrt(Horizont)) liegt der Median bei 32 Balken und
 * 58 % sind am Horizont zensiert — ein echtes Dreifach-Barrieren-Bild. Beide Lesungen sind
 * hier implementiert, KEINE ist Default: `resolveEntry` verlangt die Skalierung als
 * Argument, und Datei B muss sie benennen. Solange das nicht entschieden ist, darf Datei B
...
function buildEntries(opts) {
  const { session, stated, lastStates, lastEntryIndex, sessionIndex, closesByTicker, warmup } = opts;
  const zulassung = sessionEligible(session);
  if (!zulassung.eligible) return { rows: [], skipped: zulassung.reason, transitions: 0 };
  const wechsel = transitionsFor(lastStates, stated).filter((w) => SCORED_STATES.includes(w.to));
  const rows = [];
  for (const w of wechsel) {
    const closes = closesByTicker.get(w.ticker);
    const sigma = sigmaAtEntry(closes);
    if (sigma === null) continue;                       // ohne sigma keine Barriere, kein Eintrag
    for (const arm of HORIZONS) {
      ...
      rows.push({
        ...
        entryClose: closes[closes.length - 1], sigma63: sigma, k: K,
        m1: Number.isFinite(w.m1) ? w.m1 : null, m2: Number.isFinite(w.m2) ? w.m2 : null,
        sessionIndex, warmup: warmup === true,
      });
    }
  }
  return { rows, skipped: null, transitions: wechsel.length };
}
...
function readSchedule(t0, r1Ready) {
  const r2 = monatsEnde(plusMonate(t0, 24));
  const r3 = monatsEnde(plusMonate(t0, 36));
  const r1Frueh = monatsEnde(plusMonate(t0, 12));
  let r1 = null;
  if (r1Ready) r1 = r1Ready > r1Frueh ? monatsEnde(plusMonate(r1Ready, 0)) : r1Frueh;
...
module.exports = {
  ...
  readSchedule, nextReadDate, panelDisplay,
};
