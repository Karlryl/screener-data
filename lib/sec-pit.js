...
function pitQuarterlyWithDerivedQ4(companyFacts, concepts, { taxonomy = 'us-gaap', unit = 'USD', asOf } = {}) {
  const q = pitSeries(companyFacts, concepts, { taxonomy, unit, asOf, period: 'quarterly' });
  const a = pitSeries(companyFacts, concepts, { taxonomy, unit, asOf, period: 'annual' });
  const series = q.series.slice();
  for (const fy of a.series) {
  ...
  return { concept: q.concept || a.concept, series };
}
