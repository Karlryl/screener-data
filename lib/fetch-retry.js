 ...
 * @param {string} url - Vollstaendige HTTPS-URL.
 * @param {{headers?: Object<string, string>, pauseMs?: number, versuche?: number, backoffMs?: number, timeoutMs?: number, _get?: Function, _schlaf?: Function}} [opts] - Abrufparameter und optionale Test-Seams.
 * @throws {Error} Abruf, HTTP-Status oder bei JSON die Dekodierung scheitert.
 */
async function fetchBuffer(url, opts = {}) {
  ...
  for (let v = 0; v < versuche; v += 1) {
    ...
    if (a.code === 402) {
      // FinMind meldet ein erschoepftes Gratis-Kontingent mit 402 (live ausgeloest 19.08.2026
      // nach mehreren Vollbauten hintereinander). Die Recherche hatte das Gaeste-Limit
      // ausdruecklich als "nicht verifiziert" gefuehrt — hiermit ist es belegt. Wiederholen
      // hilft nicht, warten schon; darum eine eigene, verstaendliche Meldung statt "4xx".
      throw new Error(`${url} -> HTTP 402: Gratis-Kontingent der Quelle erschoepft. `
        + 'Nicht durch Wiederholen loesbar — spaeter erneut laufen lassen. '
        + 'Der Altbestand bleibt unangetastet.');
    }
    if (a.code < 200 || a.code >= 300) {
      // 4xx (ausser 429/402): wiederholen hilft nicht. Laut scheitern statt still leer.
 ...
 * @param {string} url - Vollstaendige HTTPS-URL.
 * @param {{headers?: Object<string, string>, pauseMs?: number, versuche?: number, backoffMs?: number, timeoutMs?: number, _get?: Function, _schlaf?: Function}} [opts] - Abrufparameter und optionale Test-Seams.
 * @returns {Promise<Object|Array<unknown>>} Geparste JSON-Antwort.
 * @throws {Error} Abruf, HTTP-Status oder bei JSON die Dekodierung scheitert.
 */
