...
/**
 * Choose the sub-profile from an explicit override, ticker, industry, or sector.
 * @param {{manualSubProfile?: string, meta?: {ticker?: string, sector?: string, industry?: string}}} stock - Snapshot metadata and optional manual override.
 * @returns { {id: string, label: string, antiManipFilters: string[]} } Profile descriptor, falling back to OTHER.
 */
function classifySubProfile(stock) {
  ...
  // audit/fix: Yahoo industry "Information Technology Services" lowercases to
  // "information technology services" which does NOT contain "it services"
  // (the "it" in "informaTIon" isn't adjacent to " services"), so Accenture/
  // Infosys/Cognizant/EPAM/Wipro/IBM-services fell through to OTHER. Match the
  // full Yahoo string via "information technology"/"technology services".
  if (ind.includes('software') || ind.includes('saas') || ind.includes('cloud') ||
      ind.includes('cybersec') || ind.includes('it services') ||
      ind.includes('information technology') || ind.includes('technology services') ||
