...
/**
 * Reject a branch range that changes a ledger alongside unrelated paths, while allowing ledger-only ranges.
 ...
 * @param {{repoDir: string, baseRef: string, headRef?: string, ledgerRel?: string, ledgerRels?: string[]}} options - Repository, comparison refs, and permitted ledger paths.
 */
 ...
 * @param {string} repoDir - Repository in which refs are resolved.
 * @param {string[]} candidates - Refs in preference order.
 * @returns {string|null} First resolvable commit ref, or null.
 * @throws {Error} Git cannot run or the repository cannot be inspected.
 */
