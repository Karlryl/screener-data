...
/**
 * Liest ein JSON-Objekt und unterscheidet eine fehlende Datei von einem defekten Bestand.
 * @param {string} p - Pfad zur JSON-Datei.
 * @returns {Object|symbol} Das gelesene Objekt oder FEHLT bei ENOENT.
 * @throws {Error} Die vorhandene Datei ist unlesbar, enthaelt ungueltiges JSON oder kein Objekt.
 */
...
/**
 * Tolerantly read optional sidecar JSON, returning null on read or parse failure.
 * Required files must keep using readJsonExistingOrThrow; parsed values are not validated.
 * @param {string|Buffer|URL} p Path of the optional JSON file.
 * @returns {*} The parsed JSON value, or null if reading or parsing fails.
 */
...
/**
 * Tolerantly read optional sidecar JSON, preserving the fallback's identity on failure.
 * Required files must keep using readJsonExistingOrThrow; parsed values are not validated.
 * @param {string|Buffer|URL} p Path of the optional JSON file.
 * @param {*} fallback Value returned unchanged when reading or parsing fails.
 * @returns {*} The parsed JSON value, or the exact fallback value on failure.
 */
function readJsonOr(p, fallback) {
  try { return JSON.parse(fs.readFileSync(p, 'utf8')); } catch (_) { return fallback; }
}

module.exports = { readJsonExistingOrThrow, FEHLT, readJsonOrNull, readJsonOr };
