...
function _classify(raw) {
  ...
  if (raw && typeof raw === 'object') {
    ...
    if (looksKeyedMap) {
      // audit F-A-2026-06-21 (Bug 26): map only the ticker-like keys, not every
      // key. Mapping over `keys` pulled metadata keys (updatedAt, note, ...) into
      // synthetic stocks and Object.assign spread their string values into
      // char-indexed props. Use the already-filtered `tickerLike` set instead.
      return { shape: 'object', stocks: tickerLike.map(k => Object.assign({ ticker: k }, raw[k])) };
    }
  }
  return { shape: 'invalid', stocks: null };
}
 ...
 * @param {Object|Array<Object>|null} raw - Parsed watchlist payload in a supported or unknown shape.
 * @returns {Array<Object>|null} Extracted or synthesized stocks; null for an unknown shape.
 */
 ...
 * @param {Object|Array<Object>|null} raw - Parsed watchlist payload in a supported or unknown shape.
 * @returns {string} wrapped, array, object, or invalid.
 */
 ...
 * @param {string} filePath - Local JSON watchlist file.
 * @returns { {shape: string, stocks: Array<Object>, size: number, raw: Object|Array<Object>|null, error: string|null} } Normalized result with visible read or schema errors.
 */
