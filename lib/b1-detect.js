...
/**
 * Ordnet ein ISO-Periodenende seinem Kalenderquartal zu.
 * @param {string} endIso - Periodenende YYYY-MM-DD.
 * @returns {string} Kalenderquartal im Format YYYYQn.
 */
function calQuarter(endIso) { return endIso.slice(0, 4) + 'Q' + (Math.floor((+endIso.slice(5, 7) - 1) / 3) + 1); }
/**
 * Liest das geklemmte Naechster-Rang-Quantil einer bereits sortierten Reihe.
 * @param {number[]} sorted - Aufsteigend sortierte Werte.
 * @param {number} p - Quantilsanteil.
 * @returns {number|null} Quantilwert oder null fuer die leere Reihe.
 */
...
/**
 * Bildet aus PIT-Umsatz und operativem Ergebnis die auswertbaren B1-Quartalszeilen einer CIK.
 * @param {{factsForCik: function(number): Object}} store - Geoeffneter Companyfacts-Store.
 * @param {number} cik - Unternehmenskennung.
 * @param {string} eraStart - Erstes zugelassenes Periodenende.
 * @param {string} eraEnd - Letztes zugelassenes Periodenende und PIT-Stichtag.
 * @param {Object<string, number>} counters - Wird um Auswertungs- und Ausfallzaehler erhoeht.
 * @returns {Array<Object>} Auswertbare Quartale mit dYoY, dYoYprev und dOpMargin; leer bei unlesbaren Facts.
 */
...
/**
 * Sammelt B1-Quartalszeilen und Ausfallzaehler fuer die ausgewaehlten CIKs.
 * @param {{factsForCik: function(number): Object}} store - Geoeffneter Companyfacts-Store.
 * @param {{eraStart: string, eraEnd: string, ciks: number[]}} options - Auswertungsfenster und Unternehmenskennungen.
 * @returns { {records: Array<Object>, counters: Object<string, number>} } Zeilen und gemeinsam fortgeschriebene Zaehler.
 */
...
/**
 * Gruppiert B1-Zeilen nach Kalenderquartal und markiert Ereignisse an den Kohortenschwellen.
 * @param {Array<Object>} records - Quartalszeilen; Ereignis- und Winsor-Felder werden an den Objekten ergaenzt.
 * @param {{minCohortN: number, pctl: number, winsorLo: number, winsorHi: number}} opts - Mindestbesetzung, Quantil und Winsor-Grenzen.
 * @returns {Object<string, {n: number, p90: number|null, belowMinN: boolean, events: Array<Object>}>} Kohorten mit Ereigniszeilen.
 */
...
/**
 * Liest alle CIK-Kennungen aus dem zentralen Verzeichnis des lokalen Companyfacts-ZIPs.
 * @param {string} [zipPath] - Lokale ZIP-Datei; sonst der Standardpfad.
 * @returns {number[]} CIKs einschliesslich historischer Unternehmen.
 * @throws {Error} Das ZIP-Verzeichnis ist nicht lesbar oder ungueltig.
 */
