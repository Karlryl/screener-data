...
/**
 * Berechnet den Median einer Zahlenreihe ohne die Eingabe zu sortieren.
 * @param {number[]} values - Ausgangswerte; nur endliche Werte werden verwendet.
 * @returns {number|null} Median oder null ohne endliche Werte.
 */
 ...
 * @param {number[]} revenueQ - Umsatzquartale, neuestes zuerst.
 * @param {string[]} revenueQEnds - Zugehoerige Quartalsenden, neuestes zuerst.
 * @param {{quarterMinGapDays: number, quarterMaxGapDays: number}} cfg - Erlaubte Abstaende der Quartalsenden.
 * @returns { {qoqLatest: number, trendQoq: number, surprise: number, quarterEnd: string}|null } Juengste QoQ-Rate gegen den Median ihrer drei Vorgaenger; null bei unbrauchbarer Reihe.
 */
...
/**
 * Vergleicht Umsatz-Trendueberraschungen innerhalb gleicher Quartalsenden und prueft Report-Frische.
 * @param {string} board - Board-Kennung.
 * @param {string} track - Track-Kennung.
 * @param {Array<Object>} rows - PIT-Zeilen mit Umsatzquartalen und Quartalsenden.
 * @param {string} now - Auswertungsdatum.
 * @param {Object<string, number>} cfg - Kohorten-, Trend- und Report-Grenzen.
 * @param {function(string): (Object|null)} earningsOf - Lookup fuer Report-Datum und dessen Quelle.
 * @returns { {track: string, poolSize: number, groups: Array<Object>, triggered: Array<Object>, skipped: Object, dataGaps: Array<Object>} } Kandidaten und nachvollziehbare Ausfallgruende.
 */
...
// verschwinden.
/**
 * Baut den Report-Datumslookup mit Vorrang fuer PIT-Daten und ausgewiesenem Live-Kalender-Fallback.
 * @param {Array<{vintage: Object}>} boards - Geladene Board-Vintages.
 * @param {string} baseDir - Verzeichnis des optionalen earnings-calendar.json.
 * @returns {Function} Tickerlookup mit pitCount und liveAvailable; liefert Datumsobjekt oder null.
 * @throws {Error} Ein vorhandener Live-Kalender ist unlesbar oder korrupt.
 */
...
/**
 * Formatiert den Earnings-Blowout samt Trend-Abweichung und Herkunft des Report-Datums.
 * @param {{ticker: string, board: string, track: string, qoqLatest: number, trendQoq: number, surprise: number, quarterEnd: string, surpriseThreshold: number, cohortN: number, reportAgeDays: number, earningsDateSource: string}} a - Ausgewerteter Alarmkandidat.
