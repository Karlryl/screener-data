...
function runE1(opts) {
  ...
  for (const f of files) {
    ...
    catch (e) { invalidBoards.push({ file: f, reason: 'unlesbar: ' + e.message }); }
  ...
  if (!directoryExists || boards.length === 0) {
    ...
    console.error('::error::e1-compression nicht messbar: ' + reason + ' (' + dir + ')');
    if (!opts.noWrite) {
    ...
    return { date, measurable: false, boardsRead: boards.length, invalidBoards,
      alerts: [], dataGaps: [], lines: [], report, statePath, outPath, exitCode: 1 };
  }
