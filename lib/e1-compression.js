 ...
 *   node lib/e1-compression.js [--date YYYY-MM-DD]
 * NUR dieser CLI-Einstieg setzt annotate:true und schreibt damit ::error::/::warning::
 * in Spalte 0. Als Bibliothek (runE1({...})) bleibt der Lauf stumm — die Wahrheit steht
 * in measurable/exitCode/invalidBoards. Wer aus einem Skript annotieren will: runE1({ annotate: true }).
 */
const fs = require('fs');
...
/**
 * Bestimmt die gerundete Tagesdifferenz zwischen zwei ISO-Datumsangaben.
 * @param {string} aIso - Startdatum.
 * @param {string} bIso - Enddatum.
 * @returns {number|null} Tagesdifferenz oder null bei ungueltigem Datum.
 */
...
/**
 * Prueft, ob eine n/m-Abdeckung vollstaendig und der Nenner positiv ist.
 * @param {string} cov - Abdeckungsangabe wie 3/3.
 * @returns {boolean} Ob Zaehler und positiver Nenner gleich sind.
 */
...
/**
 * Bestimmt das Naechster-Rang-Quantil nach Entfernen nicht endlicher Werte.
 * @param {number[]} values - Unsortierte Ausgangswerte.
 * @param {number} p - Quantilsanteil, an die verfuegbaren Raenge geklemmt.
 * @returns {number|null} Quantilwert oder null ohne endliche Werte.
 */
...
/**
 * Liest den endlichen Prozentwert einer benannten Wachstumsachse.
 * @param {Array<{key: string, pct: number}>} axisBreakdown - Achsenwerte der Board-Zeile.
 * @param {string} key - Gesuchter Achsenschluessel.
 * @returns {number|null} Prozentwert oder null ohne brauchbaren Treffer.
 */
...
/**
 * Ergaenzt fehlende Alarmparameter aus den dokumentierten Modul-Standardwerten.
 * @param {Object<string, number>} [cfg] - Optionale Ueberschreibungen der numerischen Parameter.
 * @returns {Object<string, number>} Vollstaendige Alarmkonfiguration.
 */
...
/**
 * Prueft Kompression, Wachstumsachsen und Datenfrische innerhalb eines Board-Tracks.
 * @param {string} board - Board-Kennung.
 * @param {string} track - Track-Kennung.
 * @param {Array<Object>} rows - Board-Zeilen mit ticker, coverageAxes, axisBreakdown und pit.
 * @param {string} now - Auswertungsdatum.
 * @param {Object<string, number>} cfg - Schwellen und Freshness-Grenzen.
 * @returns { {track: string, rankablePoolSize: number, thresholdEvSales: number|null, triggered: Array<Object>, exited: string[], skipped: Object, dataGaps: Array<Object>, lowN: boolean} } Kandidaten, beobachtete Austritte und Ausfallgruende.
 */
...
/**
 * Wertet profitable und unprofitable Kohorten eines Board-Vintages getrennt aus.
 * @param {{board: string, date: string, cohort: {profitable?: Array<Object>, unprofitable?: Array<Object>}}} vintage - Board-Vintage mit PIT-Zeilen.
 * @param {{cfg?: Object, date?: string, now?: string, earningsOf?: Function}} [ctx] - Auswertungsdatum, Konfiguration und bei E2 Report-Datumslookup.
 * @returns { {board: string, date: string, tracks: Object<string, Object>} } Ergebnisse beider Tracks.
 */
...
/**
 * Formatiert einen Kompressionsalarm mit ausdruecklicher EV/Sales-Kennzeichnung.
 * @param {{ticker: string, board: string, track: string, evSales: number, revGrowthLevelPct: number, revAccelerationPct: number}} a - Alarmkandidat.
 * @param {number} eigenHistorieDay - Tag der aufgebauten Eigenhistorie.
 * @returns {string} Menschenlesbare Alarmzeile.
 */
...
/**
 * Beschreibt die noch offenen Kalibrierungspunkte mit ihren aktuell verwendeten Werten.
 * @param {Object<string, number>} cfg - Aktuelle Alarmkonfiguration.
 * @returns {Array<{id: string, frage: string, status: string, aktuell: string}>} Offene Punkte fuer den Report.
 */
...
// F-CGPT-007 (P0-Haertung 2026-08-09): der frueher hier stehende `catch (_) { /* neu
// anlegen */ }` machte aus einer VORHANDENEN, kaputten Zustandsdatei eine Erstanlage —
// Schritt 6 unten schrieb den frischen Leerzustand per writeJsonAtomic darueber, und damit
// waren Eigenhistorie-Uhr, Cooldowns und alle Ticker-Zustaende weg, gemeldet als "0 Alarme",
// Exit 0. Fehlende Datei = echte Erstanlage (legitim), vorhandene kaputte = laut scheitern.
...
/**
 * Liest die Tages-Boards, wendet Alarm- und Dedup-Gates an und schreibt optional Report und State.
 * @param {{baseDir?: string, date?: string, now?: string, cfg?: Object, statePath?: string, outPath?: string, annotate?: boolean, noWrite?: boolean}} [opts] - Lokale Datenpfade, Stichtag und Steuerung von Ausgabe und Schreibvorgaengen.
 * @returns { {date: string, measurable: boolean, boardsRead: number, invalidBoards: Array<Object>, alerts: Array<Object>, dataGaps: Array<Object>, lines: string[], report: Object, statePath: string, outPath: string, exitCode: number} } Report samt Messbarkeitsstatus; fehlende Boards ergeben Exitcode 1.
 * @throws {Error} Ein vorhandener State ist korrupt oder ein Datei-Lese-/Schreibvorgang scheitert.
 */
function runE1(opts) {
  opts = opts || {};
  ...
  // Review-Nachzug Welle 7: GitHub-Annotationen (::error::/::warning::) gehoeren dem
  // CLI-Einstieg. Als Bibliothek aufgerufen — z. B. aus gruenen Tests — flutete runE1
  // sonst jeden Gate-Lauf mit roten Zeilen, die keinen Ausfall belegen. Die Rueckgabe-
  // felder (measurable, exitCode, invalidBoards) tragen die Wahrheit ueberall.
  const annotate = Boolean(opts.annotate);
  const annot = (level, msg) => {
    if (!annotate) return;
    (level === 'error' ? console.error : console.log)('::' + level + '::' + msg);
  };
  // Vor der Eingangsdaten-Pruefung validieren: ein korrupter vorhandener State
  // bleibt auch dann ein eigener harter Fehler, wenn zugleich Boards fehlen.
  ...
  const boards = [];
  const invalidBoards = [];
  // Jede gesehene Datei landet in GENAU EINEM Topf: boards ODER invalidBoards. Vorher
  // fiel eine Datei, die zu einem falsy Wert parst ("null", "0", ""), durch beide Raster
  // — sie war gesehen, aber nirgends gezaehlt, und boardFilesSeen log stillschweigend.
  ...
  // Teil-Ausfall (Auflage 7): einzelne unbrauchbare Board-Dateien duerfen nicht in einem
  // ansonsten gruenen Lauf untergehen — ein Sektor weniger heisst weniger Alarme, und das
  // saehe ohne diese Zeile wie ein ruhiger Markt aus.
  ...
  // Ohne wenigstens ein lesbares Board ist der Alarm nicht messbar. Den Report
  // trotzdem schreiben, aber weder State lesen noch unveraendert zurueckstempeln.
  if (!directoryExists || boards.length === 0) {
    ...
    const report = {
      _doc: 'E1-Kompressions-Alarm: Eingangsdaten nicht messbar.',
      schema: 'e1-compression/v1', date, generatedAt: new Date().toISOString(),
      measurable: false, boardsRead: boards.length, boardFilesSeen: files.length,
      invalidBoards, reason, alerts: [], dataGaps: []
    };
    annot('error', 'e1-compression nicht messbar: ' + reason + ' (' + dir + ')');
    if (invalidLine) annot('warning', 'e1-compression: ' + invalidBoards.length + ' unbrauchbare Board-Datei(en) im Tagesverzeichnis.');
    if (!opts.noWrite) {
      fs.mkdirSync(path.dirname(outPath), { recursive: true });
      writeJsonAtomic(outPath, report);
      // Echte Erstanlage darf den leeren State anlegen; ein vorhandener State
      // wird beim nicht messbaren Lauf dagegen byte-identisch in Ruhe gelassen.
      if (!stateExistedAtStart) {
        fs.mkdirSync(path.dirname(statePath), { recursive: true });
        writeJsonAtomic(statePath, readState(statePath));
      }
    }
    return { date, measurable: false, boardsRead: boards.length, invalidBoards,
  ...
  if (invalidLine) {
    ...
    annot('warning', 'e1-compression: ' + invalidBoards.length + ' von ' + files.length
  ...
  return { date, measurable: true, boardsRead: boards.length, invalidBoards, eigenHistorieDay, alerts: alertObjs, suppressedByCap, skipped: skipTotals, dataGaps, lowNGroups, lines, report, statePath, outPath, exitCode: 0 };
}
...
if (require.main === module) {
  ...
  opts.annotate = true;   // nur der CLI-Einstieg annotiert (siehe runE1)
  try {
    const res = runE1(opts);
    res.lines.forEach((l) => console.log(l));
    process.exit(res.exitCode);
  } catch (e) {
    console.error('::error::e1-compression FEHLER: ' + (e && e.stack || e));
    process.exit(1);
  }
}
