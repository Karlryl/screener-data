...
function evaluateTrack(board, track, rows, now, cfg) {
  ...
  if (pool.length < cfg.minCohortN) {
    return { track, rankablePoolSize: pool.length, thresholdEvSales: null, triggered: [], skipped, dataGaps, lowN: rows.length > 0 };
  }
  ...
  return { track, rankablePoolSize: pool.length, thresholdEvSales, triggered, skipped, dataGaps, lowN: false };
}
...
function runE1(opts) {
  ...
  const candSet = new Set(candidates.map((c) => c.ticker));
  const fired = [];
  ...
  // Austritt: Ticker, die heute NICHT Kandidat sind, verlassen das Quartil → Reset,
  // damit ein Wiedereintritt frisch feuern darf (Court: "nach Austritt und Wiedereintritt").
  for (const tk of Object.keys(state.tickers)) {
    if (!candSet.has(tk)) {
