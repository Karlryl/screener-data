...
function readState(statePath) {
  const s = readJsonExistingOrThrow(statePath);   // fehlt -> null, kaputt -> wirft
  if (fs.existsSync(statePath)) {
    // Vorhanden, aber kein Zustandsobjekt ('null', '[]', '5' parsen sauber durch und
    // saehen fuer den Rest der Funktion wie ein leerer Zustand aus): gleiche Klasse.
    if (!s || typeof s !== 'object' || Array.isArray(s)) {
      const err = new Error(statePath + ' enthaelt keinen E1-Zustand ('
        + (s === null ? 'null' : Array.isArray(s) ? 'Array' : typeof s)
        + ') — kein Erstanlage-Fall, refusing to overwrite');
      err.jsonPath = statePath;
      throw err;
    }
    s.tickers = s.tickers || {};
    return s;
