...
// LR-14 - DER EINE AUFLOESER. Nach dem R14a-Rollover ist das Zugriffs-Register
// ZWEI Dateien: die geschlossene und ihre Fortsetzung. Wo dieser Pfad steht,
// stand er bisher als getippte Zeichenkette in jedem Aufrufer - und zwei Kopien
// derselben Regel driften. Deshalb lebt die geordnete Menge HIER, an einer
// Stelle, und wird importiert.
//
// Die Reihenfolge ist bedeutungstragend: [0] ist die zuerst beschriebene Datei,
// das letzte Element ist die AKTIVE. Ein Anhang geht immer in die aktive Datei;
// gelesen wird ueber die ganze Kette, denn ein Eintrag aus der Zeit vor der
// Naht liegt fuer immer in der geschlossenen Datei und wird dort auch gesucht.
const REGISTER_RELS = Object.freeze([
  'protocol/early-detection/2.0.0/outcome-access-ledger.json',
  'protocol/early-detection/2.0.0/outcome-access-ledger-teil2.json',
]);
const AKTIVES_REGISTER_REL = REGISTER_RELS[REGISTER_RELS.length - 1];

/**
 * Datenformen fuer die vorhandenen Verfassungs- und Registerpruefungen.
 * @typedef {null|boolean|number|string|RegisterJson[]|Object<string, RegisterJson>} RegisterJson
 * @typedef {{typ?: string, type?: string, runId?: string, registeredAt?: string, accessedAt?: string|null, previousHash?: string, eventHash?: string, verboten?: RegisterJson}} RegisterEintrag
 * @typedef {{schema: string, genesisSha256: string, events?: RegisterEintrag[], vorgaengerDatei?: string, vorgaengerLetzteAnmeldung?: string}} ZugriffsRegister
 * @typedef {{von: string, bis: string, versiegelt?: boolean}} StudienFenster
 * @typedef {{verboteneKlassen?: string[], signalfamilie: string, verbrauchtAm: string, regel: string}} EndpunktSperre
 * @typedef {{version: string, fenster?: Object<string, StudienFenster>, oeffnungsprotokollMarke?: string, regeln?: {id: string, endpunktSperren?: EndpunktSperre[]}[]}} Regelwerk
 * @typedef {{standId: string, beobachtetAm: string}} BeobachtungsStand
 * @typedef {{laufId: string, modus: string, ergebnisdatenBeruehrt: boolean, geleseneEndpunkte: string[], regelwerkVersion: string}} LaufProtokoll
 */

// LRA-2 - DER SCHLIESSUNGSRIEGEL, EIN REINES PRAEDIKAT.
//
// Eine Registerdatei, deren letzter Akt jeden weiteren Eintrag verbietet, ist
// GESCHLOSSEN. Bis hierher stand dieses Verbot nur als Satz IM Register - kein
// Code las ihn. `anmelden` zeigte weiter auf die geschlossene Datei und schrieb
// dort erfolgreich hinein: rc=0, Erfolgs-JSON, "JETZT: committen und pushen".
// Eine geladene Waffe mit gruener Ausgabe.
//
// Das Praedikat ist REIN: es nimmt ein geparstes Register und antwortet. Es
// liest nichts, schreibt nichts und entscheidet nicht, WO gerufen wird - der
// Aufrufer sitzt am SCHREIB-Rand (scripts/studie-r1-serverzeit.js,
// schreibeRegister), nicht in pruefeZugriffsRegister und nicht in
// haengeEintragAn. Beide waeren Griffe in die Geschichtsleser-Klasse: sie
// laufen ueber JEDES gelesene Register, auch ueber die historischen Staende der
// Werkbaenke, und faerbten dort rot, ohne dass irgendetwas geschrieben wuerde.
//
// Folge, ausdruecklich: IM SPEICHER anhaengen bleibt erlaubt (die Werkbaenke
// brauchen genau das), PERSISTIEREN wird unmoeglich.
/**
 * Erkennt das Verbot weiterer Eintraege anhand des letzten Registerakts.
 * @param {ZugriffsRegister|null|undefined} register - Geparstes Register, dessen letzter Eintrag auf den Schliessungssatz geprueft wird.
 * @returns {boolean} True, wenn der letzte Akt einen Eintrag in dieser Datei verbietet.
 */
function istGeschlossen(register) {
  const events = (register && register.events) || [];
  const letzter = events[events.length - 1];
  if (!letzter) return false;
  const verboten = typeof letzter.verboten === 'string'
    ? letzter.verboten
    : JSON.stringify(letzter.verboten || '');
  // Geprueft wird die EIGENSCHAFT (das Verbot weiterer Eintraege in DIESER
  // Datei), nicht eine Kennung des Akts - ein spaeterer Abschluss-Akt mit
  // anderem Namen traegt denselben Satz und muss denselben Riegel ausloesen.
  return /Eintrag in DIESER Datei/i.test(verboten);
}

/**
 * Der repo-relative Pfad der Registerdatei, die `runId` fuehrt - oder null.
 * Ueber die KETTE gesucht, nicht am aktiven Ende geraten: der Beweis zu einem
 * Eintrag der geschlossenen Datei muss gegen GENAU DIESE Datei gefuehrt werden.
 * `lies` bekommt einen repo-relativen Pfad und liefert das geparste Register
 * oder null, wenn es die Datei nicht gibt - so bleibt diese Funktion frei von
 * Dateisystem- und Netz-Annahmen und ist gegen einen Fixture-Baum testbar.
 */
// MEHRDEUTIGKEIT IST EIN ABBRUCH, KEIN ERSTTREFFER. Traegt mehr als eine
// Registerdatei dieselbe runId, ist nicht entscheidbar, welcher Eintrag gemeint
// ist - und ein Beweis gegen die zufaellig erste waere ein sauberes Verdikt aus
// dem falschen Grund. Die Anmeldung soll das fuer neue Eintraege verhindern,
// aber ein Aufloeser, der sich auf eine Zusage ueber kuenftiges Verhalten
// verlaesst, misst nichts. Nicht gefunden ist null; mehrfach gefunden wirft.
/**
 * Ermittelt die eindeutige Registerdatei einer Laufkennung ueber die gesamte Registerkette.
 * @param {string} runId - Laufkennung, die in den Registereintraegen gesucht wird.
 * @param {function(string): (ZugriffsRegister|null)} lies - Leser fuer repo-relative Registerpfade; liefert null bei fehlender Datei.
 * @returns {string|null} Repo-relativer Pfad der einzigen Fundstelle oder null ohne Treffer.
 * @throws {Error} VerfassungsBruch, wenn mehrere Registerdateien dieselbe Laufkennung fuehren.
 */
function registerPfadDerRunId(runId, lies) {
  const treffer = REGISTER_RELS.filter((rel) => {
    const register = lies(rel);
    return !!register && (register.events || []).some((e) => e.runId === runId);
  });
  if (treffer.length > 1) {
    throw new VerfassungsBruch(
      `R1: runId ${runId} steht in MEHREREN Registerdateien (${treffer.join(', ')}). `
      + 'Welcher Eintrag gemeint ist, ist damit nicht entscheidbar.',
    );
  }
...
/**
 * Serialisiert Registerdaten mit sortierten Schluesseln und den festgelegten Python-kompatiblen Trennern.
 * @param {RegisterJson|undefined} wert - JSON-kompatibler Registerwert; undefined wird wie null geschrieben.
 * @returns {string} Kanonischer JSON-Text fuer die Verkettung der Registereintraege.
 * @throws {Error} VerfassungsBruch bei nicht-endlichen Zahlen, negativer Null oder Exponentialschreibweise.
 */
...
/**
 * Bildet den SHA-256-Fingerabdruck eines UTF-8-Textes.
 * @param {string} text - Exakter Text, dessen UTF-8-Bytes gehasht werden.
 * @returns {string} Hexadezimaler SHA-256-Hash mit Kleinbuchstaben.
 */
...
/**
 * Liest und parst die JSON-Datei der Studien-Verfassung.
 * @param {string} [pfad=REGELWERK_PFAD] - Dateipfad des zu ladenden Regelwerks.
 * @returns {Regelwerk} Geparstes Regelwerk ohne zusaetzliche Schemapruefung.
 * @throws {Error} Wenn die Datei nicht lesbar oder ihr Inhalt kein gueltiges JSON ist.
 */
...
/**
 * Liefert den ausschliesslich per Umgebungsvariable gesetzten Speicherort der Studiendaten.
 * @returns {string} Wert der Umgebungsvariable EARLY_DETECTION_DATA_ROOT.
 * @throws {Error} VerfassungsBruch, wenn die Umgebungsvariable leer oder nicht gesetzt ist.
 */
...
/**
 * Wandelt einen Quartalsschluessel in eine fortlaufende Vergleichszahl um.
 * @param {string} quartal - Quartal im Format YYYYq1 bis YYYYq4; Grossschreibung von Q wird akzeptiert.
 * @returns {number} Vierfaches Jahr plus nullbasierter Quartalsindex.
 * @throws {Error} VerfassungsBruch bei einem ungueltigen Quartalsformat.
 */
...
/**
 * Prueft, ob ein Quartal im Modusfenster liegt und ein versiegeltes Fenster geoeffnet werden darf.
 * @param {string} modus - Name des im Regelwerk definierten Studienfensters.
 * @param {string} quartal - Angefragtes Quartal im Format YYYYq1 bis YYYYq4.
 * @param {{regelwerk?: Regelwerk, oeffnungsprotokoll?: string}} [optionen={}] - Optionales Regelwerk und erforderliche Oeffnungsmarke fuer versiegelte Fenster.
 * @returns {boolean} True bei zulaessigem Quartal und erfuellter Oeffnungsbedingung.
 * @throws {Error} VerfassungsBruch bei unbekanntem Modus, ungueltigem Quartal, Fensterverletzung oder fehlender Oeffnungsmarke.
 */
...
/**
 * Erzeugt einen protokollierten Lauf mit getrennten Schranken fuer Signal- und Ergebniszugriffe.
 * @param {{laufId: string, modus: string, register?: ZugriffsRegister|null, regelwerk?: Regelwerk|null}} optionen - Laufkennung, Studienmodus sowie optional das Zugriffsregister und Regelwerk.
 * @returns {{protokoll: function(): LaufProtokoll, leseSignaldaten: function(string): boolean, leseErgebnisdaten: function(string): boolean}} Lauf-API mit kopiertem Protokoll und pruefenden Zugriffsmethoden; die Methoden lesen selbst keine Nutzdaten.
 * @throws {Error} VerfassungsBruch bei fehlender Laufkennung; spaetere Methodenaufrufe werfen bei Fenster-, Endpunkt- oder Anmeldungsverstoessen.
 */
...
/**
 * Liest die Endpunkt-Klassensperren aus der Regel R4.
 * @param {Regelwerk} regelwerk - Regel-Registry, in der die erste Regel mit id R4 gesucht wird.
 * @returns {EndpunktSperre[]} Hinterlegte Sperrliste oder ein leeres Array, wenn R4 keine Sperren fuehrt.
 */
...
/**
 * Verhindert Ergebniszugriffe auf bereits verbrauchte Endpunktklassen.
 * @param {string} endpunkt - Endpunktname, in dem verbotene Klassen ohne Beachtung der Grossschreibung gesucht werden.
 * @param {Regelwerk} regelwerk - Registry mit einer nichtleeren R4-Sperrliste.
 * @returns {boolean} True, wenn keine gesperrte Klasse als Teilstring vorkommt.
 * @throws {Error} VerfassungsBruch bei fehlendem Endpunkt, leerer Sperrliste oder Treffer in einer gesperrten Klasse.
 */
...
/**
 * Prueft den Erstzugriff gegen die Serverbestaetigung und die angemeldete frueheste Zugriffszeit.
 * @param {{serverConfirmedAt: string, ersterZugriffAm: string, accessedAt?: string|null}} zeiten - Serverbestaetigung, tatsaechlicher Erstzugriff und optional vorab angemeldeter Zugriff als lesbare Zeitangaben.
 * @returns {boolean} True, wenn der Erstzugriff strikt nach der Serverbestaetigung und nicht vor der angemeldeten Zugriffszeit liegt.
 * @throws {Error} VerfassungsBruch bei unlesbaren Zeitangaben oder verletzter zeitlicher Reihenfolge.
 */
...
/**
 * Waehlt den juengsten Beobachtungsstand, der zum Signalzeitpunkt bereits existierte.
 * @param {BeobachtungsStand[]|null|undefined} staende - Verfuegbare Staende mit Kennung und Beobachtungszeitpunkt.
 * @param {string} signalZeitpunkt - Lesbare Zeitangabe als inklusive Grenze des verfuegbaren Wissens.
 * @returns {BeobachtungsStand} Originaler Eintrag des juengsten zulaessigen Standes.
 * @throws {Error} VerfassungsBruch bei unlesbaren Zeitangaben oder wenn kein Stand rechtzeitig existierte.
 */
...
/**
 * Kopiert eine Datenzeile und versieht sie mit der Herkunft ihres Beobachtungsstands.
 * @param {Object<string, RegisterJson>} zeile - Datenzeile, deren uebrige Felder unveraendert uebernommen werden.
 * @param {BeobachtungsStand} stand - Herkunftsstand mit erforderlicher standId und Beobachtungszeitpunkt.
 * @returns {Object<string, RegisterJson> & {standId: string, standBeobachtetAm: string}} Neue Zeile mit ueberschriebenen Herkunftsfeldern.
 * @throws {Error} VerfassungsBruch, wenn der Stand oder seine Kennung fehlt.
 */
...
/**
 * Liefert eine unabhaengige Kopie der Eintragsarten mit verpflichtender Zugriffszeit.
 * @returns {Set<string>} Neues Set der zugriffszeitpflichtigen Arten ohne schreibbaren Zugriff auf die interne Menge.
 */
...
/**
 * Prueft Schema, Hashkette, Eintragsarten und zeitliche Reihenfolge eines Zugriffsregisters.
 * @param {ZugriffsRegister} register - Geparstes Register samt Genesis-Hash und gegebenenfalls zeitlicher Rollover-Verankerung.
 * @returns {{eventCount: number, tailHash: string}} Zahl der geprueften Eintraege und Hash des letzten Eintrags beziehungsweise der Genesis.
 * @throws {Error} VerfassungsBruch bei Schema-, Ketten-, Art- oder Zeitregelverletzungen.
 */
function pruefeZugriffsRegister(register) {
  ...
  // G12 - NAHT-MONOTONIE, FAIL-CLOSED.
  //
  // Die Monotonie-Pruefung unten faengt einen rueckdatierten Eintrag nur
  // INNERHALB einer Datei: sie startet bei null. Nach dem Rollover waere der
  // ERSTE Eintrag der Fortsetzung damit frei rueckdatierbar - er hat in seiner
  // Datei keinen Vorgaenger. Das ist die einzige Angriffsflaeche, die die Naht
  // neu oeffnet.
  //
  // Traegt ein Register `vorgaengerDatei`, ist `vorgaengerLetzteAnmeldung`
  // deshalb PFLICHT und setzt die Monotonie vor. Fehlt das Feld, ist das ein
  // VerfassungsBruch - nicht ein stiller Rueckfall auf null, denn ein stiller
  // Rueckfall waere genau das Loch.
  //
  // OHNE `vorgaengerDatei` verhaelt sich dieser Code BIT FUER BIT wie zuvor.
  let letzteAnmeldung = null;
  if (register.vorgaengerDatei) {
    if (!register.vorgaengerLetzteAnmeldung) {
      throw new VerfassungsBruch(
        'R1: Register fuehrt vorgaengerDatei, aber kein vorgaengerLetzteAnmeldung. '
        + 'Ohne die vorgesetzte Anmeldezeit waere der erste Eintrag der '
        + 'Fortsetzung frei rueckdatierbar (G12).',
      );
    }
    letzteAnmeldung = Date.parse(register.vorgaengerLetzteAnmeldung);
    if (Number.isNaN(letzteAnmeldung)) {
      throw new VerfassungsBruch(
        `R1: vorgaengerLetzteAnmeldung ist keine lesbare Zeit (${register.vorgaengerLetzteAnmeldung})`,
      );
    }
  }
...
/**
 * Erzeugt im Speicher eine Registerkopie mit einem neu verketteten Eintrag.
 * @param {ZugriffsRegister} register - Bestehendes Register, dessen bisherige Kette vor dem Anhaengen geprueft wird.
 * @param {RegisterEintrag} event - Anzuhaengender Eintrag; previousHash und eventHash werden neu gesetzt.
 * @returns {ZugriffsRegister} Neue Registerhuelle mit erweitertem Ereignisarray, ohne Datei-Schreibzugriff oder Validierung des neuen Eintrags.
 * @throws {Error} Wenn das bisherige Register oder die Kanonisierung des neuen Eintrags ungueltig ist.
 */
