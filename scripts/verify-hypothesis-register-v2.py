#!/usr/bin/env python3
"""Verify the append-only, proposal-only V2 hypothesis register contract."""
...
CONTRACT = ROOT / "research" / "early-detection-v4" / "hypothesis-register-contract-v2.json"
SCRIPT = Path(__file__).resolve()
TEST = ROOT / "tests" / "verify-hypothesis-register-v2.test.js"
...
EXPECTED_CONTRACT_RAW = "6c12b139edf61757dc9c457d75a4647974bb13da8473dc1b0e4fd5d3c31e24f4"
EXPECTED_CONTRACT_SELF = "199164de8a6251409b671c669c44acc1bf2577d95f534ff7bebb4b961006e4a2"
EXPECTED_VERIFIER_NORMALIZED = "078af73a3778e538715ff2077bb24eaa230eca5ae22d97031121bb75b098bdd4"
EXPECTED_TEST_RAW = "1ef3f62ece8de2e26b422b1e7febad20e4afbe46eb9c079fe821fc37994375b8"

OWNED_PATHS = [
    "research/early-detection-v4/hypothesis-register-contract-v2.json",
    "scripts/verify-hypothesis-register-v2.py",
    "tests/verify-hypothesis-register-v2.test.js",
]
DEFERRED_FIELDS = [
    "TARGET_POPULATION",
    "EXPOSURE_DEFINITION",
...
ALLOWED_INSPECTED_FIELDS = [
    ...
    "scope.payloads",
    "scope.quarters",
    "roles.*.candidates.*.taxonomyVersion",
    "roles.*.candidates.*.periodKind",
    "roles.*.candidates.*.distinctAccessions",
    "roles.*.candidates.*.distinctEntities",
    "roles.*.candidates.*.firstAcceptedEpoch",
    "roles.*.candidates.*.lastAcceptedEpoch",
    "limitations",
]
...
FORBIDDEN_ORIGIN_TOKENS = {
    ...
    "BEST_CUTOFF", "CHOOSE_BEST_CUTOFF", "STOCK_RETURN", "RETURN_VALUE", "RETURNS",
    "PRICE", "PRICES", "ENDPOINT_VALUE", "P_VALUE", "FAVORABLE_RESULT",
...
def tokenized(value: Any) -> str:
    text = json.dumps(string_values(value), ensure_ascii=False, sort_keys=True)
...
def validate_dependencies(value: dict[str, Any]) -> None:
    prior_binding = {
        ...
        for key in ("path", "bytes", "rawSha256", "gitBlob", "introductionCommit")
    }
    validate_file_binding(prior_binding, build_base=BUILD_BASE, label="prior register")
    prior = json.loads(safe_path(prior_binding["path"]).read_bytes())
    if prior.get("schema") != "early-detection-hypothesis-register/v1" or len(prior.get("hypotheses", [])) != 4:
        fail("prior register structure changed")
    if any(row.get("status") != "PROPOSAL" for row in prior["hypotheses"]):
        fail("prior register status history changed")
...
def validate_contract(value: dict[str, Any], *, exact_artifact: bool, dependencies: bool) -> None:
    ...
    if value["schema"] != "early-detection-hypothesis-register-contract/v2":
        fail("contract schema changed")
    if value["track"] != "ADDON_PROPOSALS_ONLY" or value["taskId"] != "OUTCOME-BLIND-HYPOTHESIS-REGISTER-PRE2021-V2":
    ...
    exact_keys(source_base, {
        "remote", "ref", "buildBase", "buildBaseTag", "exactRemoteAtContractBuild",
        "buildBaseMustRemainFirstParentAncestor", "interveningLinearCommitsBeforeIntroductionAllowed",
        "introductionPolicy", "introductionCommitAddsExactlyAuthorizedPaths", "authorizedPaths",
    }, "sourceBase")
    if source_base != {
        ...
        "introductionPolicy": "FIRST_REMOTE_FIRST_PARENT_DESCENDANT_THAT_ADDS_ALL_THREE_OWNED_PATHS",
        "introductionCommitAddsExactlyAuthorizedPaths": True,
        "authorizedPaths": OWNED_PATHS,
    }:
        fail("source-base topology changed")

    prior = value["priorRegister"]
    exact_keys(prior, {
        "path", "bytes", "rawSha256", "gitBlob", "introductionCommit", "proposalCount",
        "appendOnly", "unchangedAtBuildBase", "supersededForNewProposalGenerationOnly",
        "historicalDefinitionsOrStatusesMutated", "scientificCredit",
    }, "priorRegister")
    if any((
        prior["path"] != "research/early-detection-v4/hypothesis-register-v1.json",
        prior["bytes"] != 4909,
        prior["rawSha256"] != "3ffb1a62d2467faa1ae79cd1db1e220757259353003b663c45ec1b55a584ddd8",
        prior["gitBlob"] != "5aee79ee0a44e94ba38b5ba3791aac7c17bb4906",
        prior["introductionCommit"] != "59c715f2568a21142a2e1a0cb6d1c6f2e729ffba",
        prior["proposalCount"] != 4,
        prior["appendOnly"] is not True,
        prior["unchangedAtBuildBase"] is not True,
        prior["supersededForNewProposalGenerationOnly"] is not True,
    )):
        fail("V1 exact byte or Git binding changed")
    if prior["historicalDefinitionsOrStatusesMutated"] is not False or prior["scientificCredit"] != "NONE":
        fail("V1 append-only boundary changed")
    ...
    exact_keys(resume, {"path", "bytes", "rawSha256", "gitBlob", "selfField", "selfSha256", "introductionCommit"}, "resumeV2")
    if resume != {
        ...
        fail("operational resume exact byte or Git binding changed")
    ...
    if harness["introductionCommit"] != BUILD_BASE or harness["introductionTag"] != 854 or harness["requiredProviderPhase"] != "POST_INTRODUCTION":
    ...
    if boundary != {
        ...
        fail("generation boundary changed")
    ...
    if source["preOutcomeBinding"] != {"field": "limitations[2]", "exactStatement": PREOUTCOME_STATEMENT}:
        fail("generation source pre-outcome binding changed")
    ...
    if not isinstance(proposals, list) or len(proposals) != 1:
        fail("V2 must contain exactly one source-bound proposal")
    ...
    if provenance["sourceIds"] != [source["sourceId"]] or provenance["originClass"] != "THEORY_AND_SOURCE_FEATURES_ONLY":
    ...
    exact_keys(receipt, {"sourceRawSha256", "quarterCount", "firstQuarter", "lastQuarter", "maximumObservedDate", "preOutcomeStatement"}, "sourceFeatureReceipt")
    if receipt != {
        ...
        fail("source-feature receipt changed")
    ...
    scan_value = {key: val for key, val in proposal.items() if key not in {"forbiddenClaims", "proposalSha256"}}
    ...
    if event["schema"] != "early-detection-hypothesis-register-event/v2" or event["sequence"] != 1:
        fail("event schema or sequence changed")
    if event["eventId"] != "HYP-V2-EVENT-0001" or event["hypothesisId"] != proposal["hypothesisId"]:
...
def introduction_phase(*, remote: bool) -> str:
    ...
    for path in OWNED_PATHS:
        rows = git_text("log", "--format=%H", "--diff-filter=A", "--reverse", f"{BUILD_BASE}..{head}", "--", path).splitlines()
        if len(rows) != 1:
            fail("owned path introduction is not unique after build base")
...
def self_test(base_value: dict[str, Any]) -> dict[str, Any]:
    mutations: dict[str, tuple[Callable[[dict[str, Any]], None], str]] = {
        ...
        "v1Raw": (lambda x: x["priorRegister"].__setitem__("rawSha256", "1" * 64), "all"),
        "v1Blob": (lambda x: x["priorRegister"].__setitem__("gitBlob", "1" * 40), "all"),
        "v1Mutable": (lambda x: x["priorRegister"].__setitem__("historicalDefinitionsOrStatusesMutated", True), "all"),
    ...
    return {
        "schema": "early-detection-hypothesis-register-self-test/v2",
        "status": "PASS",
        "kills": kills,
        "killCount": len(kills),
        "outcomesAccessed": False,
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "self-test":
            ...
            result = {
                "schema": "early-detection-hypothesis-register-verification/v2",
                "status": "PASS",
                "phase": phase,
                "proposals": 1,
                "events": 1,
