#!/usr/bin/env python3
"""Fail-closed append-only V5 controller for the continuous no-cost source queue.
...
TEST_PATH = ROOT / "tests" / "early-detection-continuous-free-source-v5.test.js"
...
MIGRATION_PARENT_COMMIT = "b158225549caa904a467e0036d60499fd634a709"
...
base.DEFAULT_EVENTS_PATH = ROOT / "state" / "early-detection-free-source-events-v5.jsonl"
base.DEFAULT_STATE_PATH = ROOT / "state" / "early-detection-free-source-state-v5.json"
base.LOCK_PATH = ROOT / "state" / ".early-detection-free-source-controller-v5.lock"
...
def validate_v4_events(events: list[dict[str, Any]], state: dict[str, Any]) -> None:
    ...
    if base.git_bytes("show", f"{genesis_commit}:{Path(__file__).resolve().relative_to(ROOT).as_posix()}") != Path(__file__).read_bytes():
        fail("genesis commit does not bind the exact V5 controller")
    if base.git_bytes("show", f"{genesis_commit}:{TEST_PATH.relative_to(ROOT).as_posix()}") != TEST_PATH.read_bytes():
        fail("genesis commit does not bind the exact V5 test")
...
def command_init(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with base.exclusive_lock():
        if events_path.exists() or state_path.exists():
            fail("V5 init refuses existing event/state files")
