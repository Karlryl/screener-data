...
function writeExport({ outDir, exportDir, pricesDir, protocolDir, now, log }) {
  ...
  atomicWrite.writeFileAtomic(path.join(exportDir, 'regime.json'), JSON.stringify(regime) + '\n');
  atomicWrite.writeFileAtomic(path.join(exportDir, 'meta.json'), JSON.stringify(meta) + '\n');
  ...
  if (dreizehn) {
    ...
    atomicWrite.writeFileAtomic(path.join(exportDir, 'duquesne13f.json'), JSON.stringify(dreizehn) + '\n');
  ...
  if (fs.existsSync(kLedger)) {
    ...
    atomicWrite.writeFileAtomic(path.join(exportDir, 'candidates.json'), JSON.stringify(candidates) + '\n');
...
function schreibeMarkerEinmal(exportDir, grund, say) {
  ...
  if (alt !== null) {
    try {
      atomicWrite.writeFileAtomic(marker, alt);
