#!/usr/bin/env python3
"""Fail-closed append-only V4 controller for the continuous no-cost source queue.
...
TEST_PATH = ROOT / "tests" / "early-detection-continuous-free-source-v4.test.js"
...
MIGRATION_PARENT_COMMIT = "330cb89e0961d432f3533a89575b711dc82b3489"
...
base.DEFAULT_EVENTS_PATH = ROOT / "state" / "early-detection-free-source-events-v4.jsonl"
base.DEFAULT_STATE_PATH = ROOT / "state" / "early-detection-free-source-state-v4.json"
base.LOCK_PATH = ROOT / "state" / ".early-detection-free-source-controller-v4.lock"
...
def text_has_token(value: Any, tokens: set[str]) -> bool:
    ...
    return any(re.search(rf"(?<![A-Z0-9]){re.escape(token)}(?![A-Z0-9])", text) for token in normalized)
...
def validate_v4_events(events: list[dict[str, Any]], state: dict[str, Any]) -> None:
    ...
    if base.git_bytes("show", f"{genesis_commit}:{Path(__file__).resolve().relative_to(ROOT).as_posix()}") != Path(__file__).read_bytes():
        fail("genesis commit does not bind the exact V4 controller")
    if base.git_bytes("show", f"{genesis_commit}:{TEST_PATH.relative_to(ROOT).as_posix()}") != TEST_PATH.read_bytes():
        fail("genesis commit does not bind the exact V4 test")
    for event in events:
...
def command_init(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with base.exclusive_lock():
        if events_path.exists() or state_path.exists():
            fail("V4 init refuses existing event/state files")
...
def self_test() -> dict[str, Any]:
    ...
    normalization_rejected = all(text_has_token({"x": value}, FORBIDDEN_EVIDENCE_TOKENS) for value in (
        "originalV4Result", "ORIGINAL V4 RESULT", "return", "endpointValue",
    ))
