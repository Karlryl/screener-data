...
CONTRACT = ROOT / "research" / "early-detection-v4" / "sec-terminal-wealth-queue-contract-v4.json"
SCRIPT = Path(__file__).resolve()
TEST = ROOT / "tests" / "build-sec-terminal-wealth-queue-v4.test.js"
CONTRACT_RAW_SHA256 = "a776528e4ab488ddafaecf857fac8436c3c2f85731b846ef9862fb18cb4ce96c"
...
EVENT_SEQUENCE_SHA256 = "4106a7b37dff5f6b257e8247ad8bb1fffd94ffad77324bfab3f27ff1bc1cafd5"
...
def validate_contract(contract: dict, contract_raw: bytes) -> None:
    ...
    exact_keys(contract["input"], {
        ...
        "eventSequenceSha256", "databaseSidecarPolicy", "databaseOpenMode",
    }, "contract.input")
    ...
    expected_input = {
        ...
        "databaseSidecarPolicy": "ABSENT_BEFORE_DURING_AFTER_QUERY",
        "databaseOpenMode": "READ_ONLY_IMMUTABLE",
    }
    if contract["schema"] != "early-detection-sec-terminal-wealth-queue-contract/v4":
...
def sequence_digest(connection: sqlite3.Connection) -> str:
    ...
    rows = connection.execute("""
      ...
      ORDER BY e.filed_date,e.cik,e.form,e.source_payload_sha256,e.source_row_number,e.event_id,b.ticker
    """)
    for row in rows:
...
def extract(database: Path) -> list[tuple]:
    ...
    if file_state(database) != before or file_sha256(database) != DATABASE_RAW_SHA256 or file_state(database) != before:
        fail("database changed during extraction")
    require_no_sidecars(database)
    return rows
...
def build_payload(contract: dict, rows: list[tuple], contract_raw: bytes) -> dict:
    ...
    payload = {
        "schema": "early-detection-sec-terminal-wealth-queue/v4",
