#!/usr/bin/env python3
"""Capture the exact pre-sealed 122 late periodic SEC filings into a private CAS."""
from __future__ import annotations
import argparse,copy,datetime,hashlib,importlib.util,json,os,re,subprocess,time
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];CONTRACT=ROOT/'research'/'early-detection-v4'/'sec-liquidation-late-periodic-filing-capture-contract-v1.json';RUNNER=Path(__file__).resolve();TEST=ROOT/'tests'/'capture-sec-liquidation-late-periodic-filings-v1.test.js';DISCOVERY=ROOT/'scripts'/'build-sec-liquidation-late-periodic-filing-discovery-v1.py';NETWORK=ROOT/'scripts'/'capture-sec-liquidation-downstream-filings-v1.py';PRIVATE=Path(r'C:\Users\Anwender\Documents\GrowthScreenerResearchData\early-detection-v4\liquidation-late-periodic-sec-originals-v1');OWNED=(CONTRACT,RUNNER,TEST)
CONTRACT_RAW='5103770e6389987f97c94669b853732e0f356d5fd82eb095a1c1867c5b9fc5fc'
CONTRACT_SELF='386298dd5b838421390c661f49ed77cfb9dcb0bb73b224c34c3de305551f3740'
TEST_RAW='d2f592d87262c7880bf96150ba4fa33df1442cb388db5413506a074f7adf356a'
BASE='1131def14d7d9cc2380e70c8f2c2d2a12f20a60a';REMOTE='https://github.com/Karlryl/screener-data.git';REF='refs/heads/codex/early-detection-v4-gates-20260810';CREATED_AT='2026-08-13T15:42:00Z';DISCOVERY_RAW='5ee4d1467080b69e13f5c319d28c92a79b01140cc75149317de332ef57a40d11';NETWORK_RAW='4e36f022144f31defc129ec059b717a9c4a219cc49c15905ab584f5c5e9ce7d7';CANDIDATE_SHA='2576cd99eebddde4f41b2728167458c43799f3a0e4ddb038e941a95cd3d02e8c';LINK_SHA='2454b68eeb7534b95c4f7ae5da52d27e90093ccea0a30cfee1597a7e1236ce6c'
PURPOSE='Capture exactly the one hundred twenty-two pre-sealed late classic periodic SEC accounting filings once each into a private content-addressed store, without redirects, proxies, retries, public raw bytes, content interpretation, security linkage, finality, recovery, terminal wealth, price, return, outcome or Original-V4 credit.'
...
def capture_contract():return {'candidateFilings':122,'candidateCanonicalSha256':CANDIDATE_SHA,'caseCandidateLinks':519,'linkCanonicalSha256':LINK_SHA,'privateRoot':str(PRIVATE),'rawBlobLayout':'blobs/sha256/<first2>/<sha256>','receiptLayout':'receipts/<candidateId>.json','deferredLayout':'deferred/sha256/<first2>/<deferredRawSha256>.json','manifestLayout':'manifests/sha256/<first2>/<manifestSha256>.json','maximumRequestsPerRun':122,'retryCount':0,'minimumIntervalMilliseconds':250,'rateDeferredStopsRun':True,'existingBytesMustMatch':True,'atomicWriteNew':True,'publicRawBytesAllowed':False}
def validate_contract(v):
 ...
 if (v['schema'],v['createdAt'],v['taskId'],v['track'],v['purpose'])!=('sec-liquidation-late-periodic-filing-capture-contract/v1',CREATED_AT,'Q003-SEC-LIQUIDATION-LATE-PERIODIC-FILING-CAPTURE','SHARED_OUTCOME_BLIND_INFRA',PURPOSE):fail('identity changed')
...
def load_inputs(head):
 d=module('late_periodic_discovery',DISCOVERY,DISCOVERY_RAW,head);n=module('sealed_network',NETWORK,NETWORK_RAW,head);c,l,_=d.build_rows()
 if len(c)!=122 or len(l)!=519 or sha(canonical(c))!=CANDIDATE_SHA or sha(canonical(l))!=LINK_SHA:fail('population changed')
...
def build_manifest(n,candidates):
 ...
 m={'schema':'sec-liquidation-late-periodic-private-capture-manifest/v1','contractRawSha256':CONTRACT_RAW,'candidateCanonicalSha256':CANDIDATE_SHA,'expectedCandidates':122,'capturedCandidates':len(rs),'remainingCandidates':122-len(rs),'historicalDeferredEvents':len(ds),'complete':len(rs)==122,'maximumRequestSequence':events[-1][0] if events else 0,'requestEventSequenceSha256':seq([x[1] for x in events]),'receiptSequenceSha256':seq([x['receiptSha256'] for x in rs]),'rawBlobSequenceSha256':seq([x['rawSha256'] for x in rs]),'outcomesAccessed':False,'manifestSha256':''};m['manifestSha256']=n.self_hash(m,'manifestSha256');return m
def validate_manifest(n,m):
 ...
 if m['schema']!='sec-liquidation-late-periodic-private-capture-manifest/v1' or m['contractRawSha256']!=CONTRACT_RAW or m['candidateCanonicalSha256']!=CANDIDATE_SHA or m['expectedCandidates']!=122 or m['capturedCandidates']+m['remainingCandidates']!=122 or m['complete']!=(m['capturedCandidates']==122) or m['outcomesAccessed'] is not False or m['manifestSha256']!=n.self_hash(m,'manifestSha256'):fail('manifest changed')
...
def verify_repo(remote):
 ...
 for c in git('rev-list','--first-parent',f'{i}..{head}').splitlines():
  if len(git('show','-s','--format=%P',c).split())!=1:fail('later history nonlinear')
 for p in OWNED:
...
def capture():
 ...
 return {'schema':'sec-liquidation-late-periodic-filing-capture/v1','status':status,**repo,'requests':requests,'capturedCandidates':m['capturedCandidates'],'remainingCandidates':m['remainingCandidates'],'maximumRequestSequence':m['maximumRequestSequence'],'manifestSha256':m['manifestSha256'],'manifestPath':mp,'outcomesAccessed':False}
def main():
 ...
 try:
  ...
  if a.command=='capture':out=capture()
   ...
   repo=verify_repo(a.remote);n,c,l=load_inputs(git('rev-parse','HEAD'));m=build_manifest(n,c);validate_manifest(n,m);out={'schema':'sec-liquidation-late-periodic-filing-capture-check/v1','status':'PASS',**repo,'candidateFilings':len(c),'caseCandidateLinks':len(l),'capturedCandidates':m['capturedCandidates'],'remainingCandidates':m['remainingCandidates'],'historicalDeferredEvents':m['historicalDeferredEvents'],'maximumRequestSequence':m['maximumRequestSequence'],'manifestSha256':m['manifestSha256'],'networkRequests':0,'writes':0,'outcomesAccessed':False}
