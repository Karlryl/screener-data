...
CONTRACT_PATH = ROOT / "research" / "early-detection-v4" / "sec-terminal-wealth-original-inventory-contract-v4.json"
CONTRACT_RAW_SHA256 = "a59a7fc9d1f2c6e1e19b9469d32fc7852f4d899374672480b8cbc690cfdf0d76"
QUEUE_PATH = ROOT / "reports" / "early-detection" / "sec-terminal-wealth-queue-v5.json"
OUTPUT_PATH = ROOT / "reports" / "early-detection" / "sec-terminal-wealth-original-inventory-v4.json"
SCRIPT_PATH = Path(__file__).resolve()
TEST_PATH = ROOT / "tests" / "build-sec-terminal-wealth-original-inventory-v4.test.js"
...
ACCESSION_LABEL_RE = re.compile(rb"(?m)^ACCESSION NUMBER:[^\r\n]*\r?$")
ACCESSION_VALUE_RE = re.compile(rb"^ACCESSION NUMBER:[ \t]*([0-9]{10}-[0-9]{2}-[0-9]{6})[ \t]*\r?$")
QUEUE_ROW_KEYS = {
...
def load_contract() -> tuple[dict, bytes]:
    ...
    if value["schema"] != "early-detection-sec-terminal-wealth-original-inventory-contract/v4":
...
def accession_from_sec_header(raw: bytes) -> str:
    ...
    fields = list(ACCESSION_LABEL_RE.finditer(raw, start.end(), end.start()))
    if len(fields) != 1:
        fail("SEC-HEADER block must contain exactly one ACCESSION NUMBER label line")
    value = ACCESSION_VALUE_RE.fullmatch(fields[0].group(0))
    if value is None:
        fail("SEC-HEADER accession value must strictly match the accession format")
    return value.group(1).decode("ascii")
...
def validate_output(
    ...
    if payload["schema"] != "early-detection-sec-terminal-wealth-original-inventory/v4":
...
def build(blob_root: Path) -> dict:
    ...
    payload = {
        "schema": "early-detection-sec-terminal-wealth-original-inventory/v4",
...
def self_test() -> dict:
    ...
    invalid_headers = {
        ...
        "secondMalformedAccession": b"<SEC-HEADER>x\nACCESSION NUMBER: 0000000001-20-000001\nACCESSION NUMBER: MALFORMED\n</SEC-HEADER>\n",
        "secondEmptyAccession": b"<SEC-HEADER>x\nACCESSION NUMBER: 0000000001-20-000001\nACCESSION NUMBER:\n</SEC-HEADER>\n",
        "secondTrailingExtraAccession": b"<SEC-HEADER>x\nACCESSION NUMBER: 0000000001-20-000001\nACCESSION NUMBER: 0000000001-20-000002 EXTRA\n</SEC-HEADER>\n",
