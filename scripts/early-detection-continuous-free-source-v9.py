#!/usr/bin/env python3
"""Fail-closed V9 controller: exact notes and truthful lease-recovery time."""
...
TEST_PATH = ROOT / "tests" / "early-detection-continuous-free-source-v9.test.js"
...
base.DEFAULT_EVENTS_PATH = ROOT / "state" / "early-detection-free-source-events-v9.jsonl"
base.DEFAULT_STATE_PATH = ROOT / "state" / "early-detection-free-source-state-v9.json"
base.LOCK_PATH = ROOT / "state" / ".early-detection-free-source-controller-v9.lock"
...
def validate_state_paths_v9(events_path: Path, state_path: Path) -> tuple[Path, Path]:
    ...
    if events != base.DEFAULT_EVENTS_PATH.resolve() or state != base.DEFAULT_STATE_PATH.resolve():
        fail("V9 accepts only the single frozen event/state path pair")
    return events, state


def validate_contracts() -> dict:
    contracts = v8.validate_contracts()
    contracts["rawBindings"].update({
        ...
        "v8Controller": V8_SHA256,
    })
    contracts["inputBundleSha256"] = base.canonical_sha256(contracts["rawBindings"])
    return contracts


def materialize_state_v9(contracts: dict, events: list[dict], events_raw: bytes) -> dict:
...
def validate_v9_events(events: list[dict], state: dict) -> None:
    V8_VALIDATE_EVENTS(events, state)
    commit = v7.git_commit_authorized(events[0]["payload"]["baseCommit"])
    for path, expected in ((Path(__file__).resolve(), None), (TEST_PATH, None), (V8_PATH, V8_SHA256)):
        raw = path.read_bytes()
...
base.materialize_state = materialize_state_v9
base.validate_contracts = validate_contracts
v7.validate_contracts = validate_contracts
v7.validate_state_paths_v7 = validate_state_paths_v9
v8.validate_contracts = validate_contracts
v8.validate_state_paths_v8 = validate_state_paths_v9
v8.validate_v8_events = validate_v9_events


def verify_current(events_path: Path, state_path: Path, *, heal: bool=False):
    events_path,state_path=validate_state_paths_v9(events_path,state_path)
    contracts,events,state=base.verify_current(events_path,state_path,heal=heal)
    validate_v9_events(events,state)
    return contracts,events,state


def self_test() -> dict:
    contracts=validate_contracts(); created="2026-08-12T00:00:00Z"
    genesis=base.make_event(sequence=1,previous_sha=None,created_at=created,agent_id="V9-SELFTEST",run_id="V9-SELFTEST-RUN",task_id=None,event_type="QUEUE_INITIALIZED",fencing_token=0,input_bundle_sha=contracts["inputBundleSha256"],payload={"queueSeedRawSha256":contracts["rawBindings"]["queueSeed"],"repositoryRemote":v7.AUTHORIZED_REMOTE,"baseCommit":base.git_text("rev-parse","HEAD"),"controllerRawSha256":contracts["rawBindings"]["controller"],"controllerTestRawSha256":contracts["rawBindings"]["controllerTest"]})
    task_id="Q003-SEC-TERMINAL-WEALTH-QUEUE"
    claim=base.make_event(sequence=2,previous_sha=genesis["eventSha256"],created_at="2026-08-12T00:01:00Z",agent_id="V9-CLAIMER",run_id="V9-CLAIMER-RUN",task_id=task_id,event_type="TASK_CLAIMED",fencing_token=1,input_bundle_sha=contracts["inputBundleSha256"],payload={"fromState":"READY","toState":"CLAIMED","leaseExpiresAt":"2026-08-12T00:02:00Z"})
    recovery=base.make_event(sequence=3,previous_sha=claim["eventSha256"],created_at="2026-08-12T00:30:00Z",agent_id="V9-RECOVERY",run_id="V9-RECOVERY-RUN",task_id=task_id,event_type="TASK_TRANSITIONED",fencing_token=1,input_bundle_sha=contracts["inputBundleSha256"],payload={"fromState":"CLAIMED","toState":"READY","reasonCode":v8.RECOVERY_REASON,"note":v8.RECOVERY_NOTE})
    events=[genesis,claim,recovery]; raw=b"".join(base.canonical_bytes(event)+b"\n" for event in events)
    state=materialize_state_v9(contracts,events,raw); validate_v9_events(events,state)
    arbitrary_notes=("outcome=positive","outcomePositive","profitability high","significant effect","win rate 75 percent","result=PASS")
    exact_templates_reject_arbitrary=all(note not in ALLOWED_TRANSITION_NOTES.values() for note in arbitrary_notes)
    return {"status":"PASS","truthfulMaterializedAt":state["materializedAt"]=="2026-08-12T00:30:00Z","expiredLeaseRecovered":next(row for row in state["tasks"] if row["taskId"]==task_id)["state"]=="READY","exactTransitionTemplatesRejectArbitraryText":exact_templates_reject_arbitrary,"v8PreImportBytesBound":hashlib.sha256(v8_source).hexdigest()==V8_SHA256,"outcomesAccessed":False}
...
def main() -> int:
    ...
    try:
        if args.command=="init": result=v7.command_init(args)
            ...
            key=None
            # Reject before constructing an event so arbitrary user/agent text never enters the log.
            contracts,events,state=verify_current(Path(args.events),Path(args.state)); task=next((row for row in state["tasks"] if row["taskId"]==args.task_id),None)
        ...
        else: fail("unsupported V9 command")
