...
// Also carries the 0.9 pull-mix (n_full/n_price_only) through to the dashboard consumer.
function buildMarker(res, m) {
  ...
  return {
    ...
    n_price_only: (m && Number.isFinite(m.n_price_only)) ? m.n_price_only : null,
...
function validateMarker(mk) {
  ...
  // n_full/n_price_only nullable (legacy manifests) but if present must be finite
  for (const f of ['n_full', 'n_price_only']) {
