...
CONTRACT = ROOT / "research" / "early-detection-v4" / "quantconnect-free-cloud-pilot-contract-v5.json"
SCRIPT = ROOT / "scripts" / "quantconnect-free-cloud-metadata-pilot-v5.py"
TEST = ROOT / "tests" / "quantconnect-free-cloud-pilot-v5.test.js"
...
def no_forbidden(value, path="root"):
    if isinstance(value, dict):
        ...
        for token in FORBIDDEN_KEYS:
            require(token not in normalized and token.replace("_", "") not in compact, f"forbidden text {path}")
...
def validate_static():
    ...
    require(contract["schema"] == "early-detection-quantconnect-free-cloud-pilot-contract/v5", "contract schema")
...
def validate_output(path, cases, contract):
    ...
    require(value["schema"] == "early-detection-quantconnect-free-cloud-metadata-output/v5", "output schema")
    ...
    for row in value["rows"]:
        ...
        require(row["errors"] == [], "row errors must remain empty")
        ...
        for index, alias in enumerate(row["aliasResults"]):
            ...
            require(alias["securityIdentifier"] is None or (isinstance(alias["securityIdentifier"], str) and bool(alias["securityIdentifier"])), "security identifier type")
            require(isinstance(alias["barCount"], int) and not isinstance(alias["barCount"], bool) and alias["barCount"] >= 0, "barCount")
            require(isinstance(alias["errors"], list) and all(isinstance(item, str) and re.fullmatch(r"[A-Za-z][A-Za-z0-9_.]{0,127}", item) for item in alias["errors"]), "alias errors")
            if alias["subscriptionAccepted"]:
                require(alias["securityIdentifier"] is not None, "accepted subscription requires security identifier")
            else:
                require(alias["securityIdentifier"] is None and alias["barCount"] == 0, "rejected subscription cannot carry identity or bars")
                require(alias["splitDates"] == [] and alias["dividendDates"] == [] and alias["symbolChanges"] == [] and alias["delistingEvents"] == [], "rejected subscription cannot carry events")
            if alias["barCount"] == 0:
            ...
            require(alias["symbolChanges"] == sorted(alias["symbolChanges"], key=lambda item: (item["date"], item["requestedTicker"], item["oldSymbol"], item["newSymbol"])), "symbol events order")
            require(len({canonical(item) for item in alias["symbolChanges"]}) == len(alias["symbolChanges"]), "duplicate symbol event")
            ...
            require(alias["delistingEvents"] == sorted(alias["delistingEvents"], key=lambda item: (item["date"], item["requestedTicker"], item["eventType"])), "delisting events order")
            require(len({canonical(item) for item in alias["delistingEvents"]}) == len(alias["delistingEvents"]), "duplicate delisting event")
            start_date = datetime.strptime(case["referenceStart"], "%Y-%m-%d").date()
            end_date = datetime.strptime(case["referenceEnd"], "%Y-%m-%d").date()
            weekdays = sum(1 for offset in range((end_date - start_date).days + 1) if (start_date.toordinal() + offset - 1) % 7 < 5)
            require(alias["barCount"] <= weekdays, "barCount exceeds weekday window")
    no_forbidden(value)
    return value
