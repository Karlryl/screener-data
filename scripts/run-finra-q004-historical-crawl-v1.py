...
ROOT=Path(__file__).resolve().parents[1];CONTRACT=ROOT/'research'/'early-detection-v4'/'finra-q004-historical-crawl-contract-v1.json';TEST=ROOT/'tests'/'run-finra-q004-historical-crawl-v1.test.js';OUTPUT=ROOT/'reports'/'early-detection'/'finra-q004-historical-crawl-manifest-v1.json';PARTITIONS=ROOT/'reports'/'early-detection'/'finra-q004-partitions-2009-2024-v1.json';METADATA=ROOT/'reports'/'early-detection'/'finra-q004-otc-daily-list-metadata-v1.json';TERMS=ROOT/'reports'/'early-detection'/'finra-q004-api-terms-snapshot-v2.json';PILOT_SCRIPT=ROOT/'scripts'/'run-finra-q004-single-day-pilot-v2.py';PRIVATE=Path(r'C:\Users\Anwender\Documents\GrowthScreenerResearchData\private-evidence\finra-q004\historical');CHECKPOINT=PRIVATE/'checkpoint-v1.json'
PARENT='f44ea412b9df000b4eba1f3284c58f4b6e584b43';REMOTE='origin';BRANCH='codex/early-detection-v4-gates-20260810';REMOTE_URL='https://github.com/Karlryl/screener-data.git';PARTITIONS_SHA='2772239277ff93be4e3034b0c80c13b41a080b6c1d606a7b7a3f158fb6c06b1c';METADATA_SHA='dc1ce543a2c2dbc80187d7757874245e504bb3c04b1faa8ccbdaa91a8ad23955';TERMS_SHA='686b31c58169809e627edb346e6d281437185e12dbc0065896251622814c1fe7';PILOT_SHA='0e08a44a9a75616a15407eb7b5682eed1a663168f39ed69ca5b9bed0492ca22f';DATES_SHA='4b6b991889079d3fbcf92de1a4853988d3eb78ad650ddee50cdbeba8f9381b37';DATA='https://api.finra.org/data/group/otcMarket/name/OTCDAILYLIST';HEX64=re.compile(r'^[0-9a-f]{64}$');HEX40=re.compile(r'^[0-9a-f]{40}$')
...
def validate_contract():
 ...
 if set(x)!={'schema','createdAt','track','taskId','sourceId','purpose','authorization','coverageContract','requestContract','storageContract','completionContract','claimLocks','contractSha256'}:fail('contract keys')
 if x['schema']!='finra-q004-historical-crawl-contract/v1' or x['track']!='SHARED_OUTCOME_BLIND_INFRA' or x['taskId']!='Q004-FINRA-OTC-CATALOG' or x['sourceId']!='FINRA_OTC_PRIMARY':fail('boundary')
 ...
 if x['storageContract']!={'privateRoot':PRIVATE.as_posix(),'rawPagePathTemplate':'blobs/sha256/{sha256[0:2]}/{sha256}','checkpointPath':'checkpoint-v1.json','checkpointWrite':'ATOMIC_REPLACE_LOCAL_ONLY','rawDataPublicGitAllowed':False,'resumeVerifiesEveryPriorBlob':True,'writeNewRawBlobs':True}:fail('storage')
 if x['completionContract']!={'outputPath':'reports/early-detection/finra-q004-historical-crawl-manifest-v1.json','all1522PartitionsRequired':True,'recordTotalStableWithinPartition':True,'pageOffsetsContiguous':True,'allRowsMatchPartition':True,'all60FieldsExact':True,'idsUniqueWithinAndAcrossPartitions':True,'twoPrivateRebuildsRequiredBeforePromotion':True,'rawRowsIncludedInPublicManifest':False} or any(x['claimLocks'].values()) or utc(x['createdAt'])>datetime.now(timezone.utc):fail('completion/locks/time')
 inputs();return x
 ...
 for item in x['completed']:
  ...
  for page in item['pages']:
   ...
   if deep:
    ...
    for row in rows:
     if set(row)!=set(fields) or row['calendarDay']!=item['calendarDay'] or not isinstance(row['OTCDailyListID'],int) or row['OTCDailyListID'] in ids:fail('prior row')
...
def crawl(c):
 ...
 if CHECKPOINT.exists():cp=load(CHECKPOINT);validate_checkpoint(cp,c,binding,dates,fields,True)
 ...
 try:
  for day in dates:
   ...
   while expected_total is None or offset<expected_total:
    ...
    for row in rows:
     if not isinstance(row,dict) or set(row)!=set(fields) or row['calendarDay']!=day or not isinstance(row['OTCDailyListID'],int) or row['OTCDailyListID'] in ids:fail('row')
     ids.add(row['OTCDailyListID'])
...
def validate_manifest(r,c):
 ...
 if claim!=sha(canonical(b)) or r['schema']!='finra-q004-historical-crawl-manifest/v1' or r['contractRawSha256']!=sha_file(CONTRACT) or r['contractSha256']!=c['contractSha256']:fail('manifest binding')
...
def self_test(c):
 ...
 return {'schema':'finra-q004-historical-self-test/v1','status':'PASS','kills':kills,'outcomesAccessed':False}
def main():
 ...
 try:
  c=validate_contract()
  if a.command=='verify-contract':print(json.dumps({'schema':'finra-q004-historical-contract-verification/v1','status':'PASS','contractSha256':c['contractSha256'],'outcomesAccessed':False},sort_keys=True))
   ...
   validate_checkpoint(cp,c,binding,dates,fields,True);rebuilds=[{'runId':'REBUILD_NORMAL','status':'PASS'},{'runId':'REBUILD_OPTIMIZED','status':'PASS'}];r=build_manifest(c,cp,dates,fields,rebuilds);validate_manifest(r,c);out=canonical(r)+b'\n';write_new(OUTPUT,out);print(json.dumps({'schema':'finra-q004-historical-finalize/v1','status':'PASS','output':OUTPUT.relative_to(ROOT).as_posix(),'rawSha256':sha(out),'reportSha256':r['reportSha256'],'recordCount':cp['totalRows'],'outcomesAccessed':False},sort_keys=True))
  else:
   if a.output is None or Path(a.output).resolve()!=OUTPUT.resolve():fail('path')
   ...
   print(json.dumps({'schema':'finra-q004-historical-output-verification/v1','status':'PASS','rawSha256':sha(out),'reportSha256':r['reportSha256'],'privateCasVerified':True,'recordCount':cp['totalRows'],'outcomesAccessed':False},sort_keys=True))
  return 0
 except RateDeferred:
