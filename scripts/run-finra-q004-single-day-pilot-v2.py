...
ROOT=Path(__file__).resolve().parents[1];CONTRACT=ROOT/'research'/'early-detection-v4'/'finra-q004-single-day-pilot-contract-v2.json';TEST=ROOT/'tests'/'run-finra-q004-single-day-pilot-v2.test.js';OUTPUT=ROOT/'reports'/'early-detection'/'finra-q004-single-day-pilot-v2.json';META=ROOT/'scripts'/'run-finra-q004-accounted-metadata-v1.py';META_OUT=ROOT/'reports'/'early-detection'/'finra-q004-otc-daily-list-metadata-v1.json';TERMS=ROOT/'reports'/'early-detection'/'finra-q004-api-terms-snapshot-v2.json';CAS=Path(r'C:\Users\Anwender\Documents\GrowthScreenerResearchData\private-evidence\finra-q004\data')
PARENT='d16088bdb74dbfc70a07c62dbbea7da7644a800e';REMOTE='origin';BRANCH='codex/early-detection-v4-gates-20260810';REMOTE_URL='https://github.com/Karlryl/screener-data.git';META_SHA='aee78a1dfd2b946937fed9451340bb76afa7e137e05d234a76bc1c4f0889e52a';META_OUT_SHA='dc1ce543a2c2dbc80187d7757874245e504bb3c04b1faa8ccbdaa91a8ad23955';TERMS_SHA='686b31c58169809e627edb346e6d281437185e12dbc0065896251622814c1fe7';V1_CONTRACT_SHA='72bed6052aa43e2077293dbd50efb5d66d4b322ce23c3120f1b1a382ae4c16a6'
...
def validate_contract():
 ...
 if set(x)!={'schema','createdAt','track','taskId','sourceId','purpose','authorization','query','networkContract','privateCas','outputContract','claimLocks','supersededAttempt','contractSha256'}:fail('contract keys')
 if x['schema']!='finra-q004-single-day-pilot-contract/v2' or x['track']!='SHARED_OUTCOME_BLIND_INFRA' or x['taskId']!='Q004-FINRA-OTC-CATALOG' or x['sourceId']!='FINRA_OTC_PRIMARY':fail('boundary')
 ...
 if x['networkContract']!={'tokenRequests':1,'dataRequests':1,'redirectsAllowed':False,'environmentProxyUseAllowed':False,'retryAllowed':False,'maximumResponseBytes':3145728,'maximumRows':1000,'requiredResponseHeaders':['content-type','record-limit','record-offset','record-total']}:fail('network')
 ...
 if x['outputContract']!={'path':'reports/early-detection/finra-q004-single-day-pilot-v2.json','rowValuesIncluded':False,'rawResponseIncluded':False,'secretsCaptured':False,'outcomesAccessed':False} or x['supersededAttempt']!={'contractRawSha256':V1_CONTRACT_SHA,'requestsMade':3,'publicOutputCreated':False,'disposition':'FAILED_HEADER_ASSUMPTION_ZERO_CREDIT'} or any(x['claimLocks'].values()) or utc(x['createdAt'])>datetime.now(timezone.utc):fail('output, predecessor, locks, or time')
 return x
...
def validate_rows(rows,headers):
 ...
 for row in rows:
  ...
  ids.add(ident)
 try:total=int(headers['record-total']);offset=int(headers['record-offset']);limit=int(headers['record-limit']);page=len(rows)
...
def safe_headers(headers):
 keys=['content-type','record-limit','record-offset','record-total','record-max-limit','response-payload-max-size'];return {k:headers[k] for k in keys if k in headers}
def build(c,binding,payload,raw,headers,counts,at):
 hs=safe_headers(headers);r={'schema':'finra-q004-single-day-pilot/v2','capturedAt':at,'track':c['track'],'taskId':c['taskId'],'sourceId':c['sourceId'],'contractRawSha256':sha_file(CONTRACT),'contractSha256':c['contractSha256'],'metadataEvidenceRawSha256':META_OUT_SHA,'termsEvidenceRawSha256':TERMS_SHA,'implementationBindings':binding,'supersededAttempt':c['supersededAttempt'],'requestEvidence':{'method':'POST','url':DATA,'canonicalPayloadSha256':sha(payload),'calendarDay':DAY,'fields':FIELDS,'limit':1000,'offset':0},'responseEvidence':{'httpStatus':200,'rawSha256':sha(raw),'bytes':len(raw),'privateCasRelativePath':f'sha256/{sha(raw)[:2]}/{sha(raw)}','headersCanonicalSha256':sha(canonical(hs)),'headers':hs,'rawResponseIncluded':False},'counts':counts,'rowValuesIncluded':False,'secretsCaptured':False,'outcomesAccessed':False,'claimLocks':c['claimLocks']};r['reportSha256']=sha(canonical(r));return r
def validate_report(r,c):
 if set(r)!={'schema','capturedAt','track','taskId','sourceId','contractRawSha256','contractSha256','metadataEvidenceRawSha256','termsEvidenceRawSha256','implementationBindings','supersededAttempt','requestEvidence','responseEvidence','counts','rowValuesIncluded','secretsCaptured','outcomesAccessed','claimLocks','reportSha256'}:fail('report keys')
 b=dict(r);claim=b.pop('reportSha256',None)
 if claim!=sha(canonical(b)) or r['schema']!='finra-q004-single-day-pilot/v2' or r['contractRawSha256']!=sha_file(CONTRACT) or r['contractSha256']!=c['contractSha256'] or r['metadataEvidenceRawSha256']!=META_OUT_SHA or r['termsEvidenceRawSha256']!=TERMS_SHA or r['supersededAttempt']!=c['supersededAttempt'] or utc(c['createdAt'])>utc(r['capturedAt']):fail('report binding')
...
def self_test(c):
 ...
 return {'schema':'finra-q004-single-day-pilot-self-test/v2','status':'PASS','kills':kills,'fixtureCount':counts['recordTotal'],'outcomesAccessed':False}
def main():
 ...
 try:
  c=validate_contract()
  if a.command=='verify-contract':print(json.dumps({'schema':'finra-q004-single-day-pilot-contract-verification/v2','status':'PASS','contractSha256':c['contractSha256'],'outcomesAccessed':False},sort_keys=True))
   ...
   digest=cas_write(raw);at=datetime.now(timezone.utc).replace(microsecond=0).strftime('%Y-%m-%dT%H:%M:%SZ');r=build(c,after,payload,raw,headers,counts,at);validate_report(r,c);out=canonical(r)+b'\n';write_new(OUTPUT,out);print(json.dumps({'schema':'finra-q004-single-day-pilot-write-result/v2','status':'PASS','output':OUTPUT.relative_to(ROOT).as_posix(),'rawSha256':sha(out),'reportSha256':r['reportSha256'],'privateResponseSha256':digest,'recordTotal':counts['recordTotal'],'outcomesAccessed':False},sort_keys=True))
  else:
   if a.output is None or Path(a.output).resolve()!=OUTPUT.resolve():fail('frozen output')
   ...
   print(json.dumps({'schema':'finra-q004-single-day-pilot-output-verification/v2','status':'PASS','rawSha256':sha(out),'reportSha256':r['reportSha256'],'privateCasVerified':True,'remoteVerified':a.remote,'recordTotal':r['counts']['recordTotal'],'outcomesAccessed':False},sort_keys=True))
  return 0
