#!/usr/bin/env python3
"""Fail-closed append-only V7 controller for the continuous no-cost source queue.
...
TEST_PATH = ROOT / "tests" / "early-detection-continuous-free-source-v7.test.js"
...
MIGRATION_PARENT_COMMIT = "788a4eb38386d958c03fef4fd12fe749d6954055"
...
base.DEFAULT_EVENTS_PATH = ROOT / "state" / "early-detection-free-source-events-v7.jsonl"
base.DEFAULT_STATE_PATH = ROOT / "state" / "early-detection-free-source-state-v7.json"
base.LOCK_PATH = ROOT / "state" / ".early-detection-free-source-controller-v7.lock"
...
def validate_state_paths_v7(events_path: Path, state_path: Path) -> tuple[Path, Path]:
    ...
    if events != base.DEFAULT_EVENTS_PATH.resolve() or state != base.DEFAULT_STATE_PATH.resolve():
        fail("V7 accepts only the single frozen event/state path pair")
    return events, state
...
def validate_v4_events(events: list[dict[str, Any]], state: dict[str, Any]) -> None:
    ...
    if base.git_bytes("show", f"{genesis_commit}:{Path(__file__).resolve().relative_to(ROOT).as_posix()}") != Path(__file__).read_bytes():
        fail("genesis commit does not bind the exact V7 controller")
    if base.git_bytes("show", f"{genesis_commit}:{TEST_PATH.relative_to(ROOT).as_posix()}") != TEST_PATH.read_bytes():
        fail("genesis commit does not bind the exact V7 test")
...
def verify_current(events_path: Path, state_path: Path, *, heal: bool = False, synthetic_fixture: bool = False):
    events_path, state_path = validate_state_paths_v7(events_path, state_path)
...
def command_init(args: argparse.Namespace) -> dict[str, Any]:
    ...
    events_path, state_path = validate_state_paths_v7(Path(args.events), Path(args.state))
    with base.exclusive_lock():
        if events_path.exists() or state_path.exists():
            fail("V7 init refuses existing event/state files")
...
def self_test() -> dict[str, Any]:
    ...
    normalization_rejected = all(text_has_token({"x": value}, FORBIDDEN_EVIDENCE_TOKENS) for value in (
        ...
        "seccikgrowthpersistenceanalysisledger", "SEC-CIK-GROWTH-PERSISTENCE analysis-ledger result PASS",
    ))
