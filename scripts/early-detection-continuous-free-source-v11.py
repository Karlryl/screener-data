#!/usr/bin/env python3
"""Fail-closed V11 controller: exact notes and wall-clock-safe lease recovery."""
...
V10_PATH = ROOT / "scripts" / "early-detection-continuous-free-source-v10.py"
...
V10_SHA256 = "2420011d3dc8162d031915a3ca19925547c80b0622e07d2fd50c4a30b25f17ef"
...
if hashlib.sha256(V10_PATH.read_bytes()).hexdigest() != V10_SHA256:
    raise RuntimeError("immutable V10 controller bytes changed")
...
TEST_PATH = ROOT / "tests" / "early-detection-continuous-free-source-v11.test.js"
base.SCRIPT_PATH = Path(__file__).resolve(); base.TEST_PATH = TEST_PATH
base.DEFAULT_EVENTS_PATH = ROOT / "state" / "early-detection-free-source-events-v11.jsonl"
base.DEFAULT_STATE_PATH = ROOT / "state" / "early-detection-free-source-state-v11.json"
base.LOCK_PATH = ROOT / "state" / ".early-detection-free-source-controller-v11.lock"
...
def validate_state_paths_v10(events_path: Path, state_path: Path) -> tuple[Path, Path]:
    ...
    if events != base.DEFAULT_EVENTS_PATH.resolve() or state != base.DEFAULT_STATE_PATH.resolve():
        fail("V11 accepts only the single frozen event/state path pair")
    return events, state
...
def validate_transition_templates(events: list[dict]) -> None:
    for index, event in enumerate(events):
        ...
        if payload["reasonCode"] == v8.RECOVERY_REASON:
            claim = v8.recovery_claim(events, index, event["taskId"])
            lease = base.parse_z(claim["payload"]["leaseExpiresAt"], "leaseExpiresAt")
            if datetime.now(timezone.utc) <= lease:
                fail("lease recovery rejected before actual wall-clock expiry")
...
def self_test() -> dict:
    ...
    genesis=base.make_event(sequence=1,previous_sha=None,created_at=created,agent_id="V11-SELFTEST",run_id="V11-SELFTEST-RUN",task_id=None,event_type="QUEUE_INITIALIZED",fencing_token=0,input_bundle_sha=contracts["inputBundleSha256"],payload={"queueSeedRawSha256":contracts["rawBindings"]["queueSeed"],"repositoryRemote":v7.AUTHORIZED_REMOTE,"baseCommit":base.git_text("rev-parse","HEAD"),"controllerRawSha256":contracts["rawBindings"]["controller"],"controllerTestRawSha256":contracts["rawBindings"]["controllerTest"]})
    task_id="Q003-SEC-TERMINAL-WEALTH-QUEUE"
    claim=base.make_event(sequence=2,previous_sha=genesis["eventSha256"],created_at="2026-08-12T00:01:00Z",agent_id="V11-CLAIMER",run_id="V11-CLAIMER-RUN",task_id=task_id,event_type="TASK_CLAIMED",fencing_token=1,input_bundle_sha=contracts["inputBundleSha256"],payload={"fromState":"READY","toState":"CLAIMED","leaseExpiresAt":"2026-08-12T00:02:00Z"})
    recovery=base.make_event(sequence=3,previous_sha=claim["eventSha256"],created_at="2026-08-12T00:30:00Z",agent_id="V11-RECOVERY",run_id="V11-RECOVERY-RUN",task_id=task_id,event_type="TASK_TRANSITIONED",fencing_token=1,input_bundle_sha=contracts["inputBundleSha256"],payload={"fromState":"CLAIMED","toState":"READY","reasonCode":v8.RECOVERY_REASON,"note":v8.RECOVERY_NOTE})
    ...
    now=datetime.now(timezone.utc); future_lease=(now+timedelta(minutes=4)).strftime("%Y-%m-%dT%H:%M:%SZ"); future_event=(now+timedelta(minutes=4,seconds=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
    early_claim=base.make_event(sequence=2,previous_sha=genesis["eventSha256"],created_at=now.strftime("%Y-%m-%dT%H:%M:%SZ"),agent_id="V11-EARLY",run_id="V11-EARLY-RUN",task_id=task_id,event_type="TASK_CLAIMED",fencing_token=1,input_bundle_sha=contracts["inputBundleSha256"],payload={"fromState":"READY","toState":"CLAIMED","leaseExpiresAt":future_lease})
    early_recovery=base.make_event(sequence=3,previous_sha=early_claim["eventSha256"],created_at=future_event,agent_id="V11-FOREIGN",run_id="V11-FOREIGN-RUN",task_id=task_id,event_type="TASK_TRANSITIONED",fencing_token=1,input_bundle_sha=contracts["inputBundleSha256"],payload={"fromState":"CLAIMED","toState":"READY","reasonCode":v8.RECOVERY_REASON,"note":v8.RECOVERY_NOTE})
    early_rejected=False
    try: validate_transition_templates([genesis,early_claim,early_recovery])
    except base.ControllerError: early_rejected=True
    return {"status":"PASS","truthfulMaterializedAt":state["materializedAt"]=="2026-08-12T00:30:00Z","expiredLeaseRecovered":next(row for row in state["tasks"] if row["taskId"]==task_id)["state"]=="READY","earlyForeignRecoveryRejected":early_rejected,"exactTransitionTemplatesRejectArbitraryText":arbitrary_rejected,"v8PreImportBytesBound":hashlib.sha256(v8_source).hexdigest()==V8_SHA256,"supersededV9Bound":hashlib.sha256(FAILED_V9_PATH.read_bytes()).hexdigest()==FAILED_V9_SHA256,"v10Bound":hashlib.sha256(V10_PATH.read_bytes()).hexdigest()==V10_SHA256,"outcomesAccessed":False}
