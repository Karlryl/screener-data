...
CONTRACT = ROOT / "research" / "early-detection-v4" / "quantconnect-free-cloud-pilot-contract-v4.json"
SCRIPT = ROOT / "scripts" / "quantconnect-free-cloud-metadata-pilot-v4.py"
TEST = ROOT / "tests" / "quantconnect-free-cloud-pilot-v4.test.js"
...
def validate_static():
    ...
    require(contract["schema"] == "early-detection-quantconnect-free-cloud-pilot-contract/v4", "contract schema")
...
def validate_output(path, cases, contract):
    ...
    require(value["schema"] == "early-detection-quantconnect-free-cloud-metadata-output/v4", "output schema")
    ...
    for row in value["rows"]:
        ...
        for index, alias in enumerate(row["aliasResults"]):
            ...
            require(isinstance(alias["errors"], list) and all(isinstance(item, str) and bool(item) for item in alias["errors"]), "alias errors")
            ...
            calendar_days = (datetime.strptime(case["referenceEnd"], "%Y-%m-%d") - datetime.strptime(case["referenceStart"], "%Y-%m-%d")).days + 1
            require(alias["barCount"] <= calendar_days, "barCount exceeds calendar window")
    no_forbidden(value)
    return value
