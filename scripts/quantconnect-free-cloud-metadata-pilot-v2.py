...
class FreeCloudMetadataPilotV2(QCAlgorithm):
    """Discovery-only metadata pilot; never exports prices, returns, holdings or alpha."""

    PILOT_CORE_SHA256 = "952c509f9eac982ef6f0e6016778a58e0ab1a34bf945fbf2256634cc164dd70a"
    ...
    def initialize(self):
        ...
        for case_id, category, query, alternates, start, end in self.CASES:
            row = {
                ...
                "aliasResults":[],"barCount":0,"firstBarDate":None,"lastBarDate":None,
                "splitDates":[],"dividendDates":[],"symbolChanges":[],"delistingEvents":[],"errors":[]
            }
            self._rows[case_id] = row
            for role, ticker in [("PRIMARY", query)] + [("ALTERNATE", x) for x in alternates]:
                alias = {"role":role,"requestedTicker":ticker,"subscriptionAccepted":False,"securityIdentifier":None,"errors":[]}
        ...
        for symbol, bindings in self._subscriptions.items():
            for row, alias in bindings:
                ...
                if data.bars.get(symbol) is not None:
                    row["barCount"] += 1
                    row["firstBarDate"] = row["firstBarDate"] or day
                    row["lastBarDate"] = day
                if data.splits.contains_key(symbol) and day not in row["splitDates"]:
                    row["splitDates"].append(day)
                if data.dividends.contains_key(symbol) and day not in row["dividendDates"]:
                    row["dividendDates"].append(day)
                if data.symbol_changed_events.contains_key(symbol):
                    ...
                    if item not in row["symbolChanges"]:
                        row["symbolChanges"].append(item)
                if data.delistings.contains_key(symbol):
                    ...
                    if item not in row["delistingEvents"]:
                        row["delistingEvents"].append(item)

    def on_end_of_algorithm(self):
        rows = [self._rows[key] for key in sorted(self._rows)]
        report = {
            "schema":"early-detection-quantconnect-free-cloud-metadata-output/v2",
        ...
        self.log("QC_METADATA_V2=" + json.dumps(report, sort_keys=True, separators=(",", ":")))
