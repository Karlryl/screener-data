...
const script = path.join(root, 'scripts', 'verify-twelve-data-basic-prerequisite-v1.py');
const contract = path.join(root, 'research', 'early-detection-v4', 'twelve-data-basic-prerequisite-contract-v1.json');
const EXPECTED_RAW_SHA256 = '769efda94a83a112ee8945871209da7315341908a2673c3595b804fd6fd05890';
...
for (const optimized of [false, true]) {
  ...
  for (const command of ['verify', 'self-test']) {
    ...
    if (command === 'self-test') {
      ...
      assert.equal(result.apiCreditsPerDay, 800);
...
console.log('verify-twelve-data-basic-prerequisite-v1.test.js: PASS');
