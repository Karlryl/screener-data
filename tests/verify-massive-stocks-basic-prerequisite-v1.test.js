...
const script = path.join(root, 'scripts', 'verify-massive-stocks-basic-prerequisite-v1.py');
const contract = path.join(root, 'research', 'early-detection-v4', 'massive-stocks-basic-prerequisite-contract-v1.json');
const EXPECTED_RAW_SHA256 = '95f84d1a97756186d2528f04bb7a4166234f00c55dfd569978c1cf232379e0a9';
...
for (const optimized of [false, true]) {
  ...
  for (const command of ['verify', 'self-test']) {
    ...
    assert.equal(result.pricesAccessed, false);
    ...
    if (command === 'self-test') {
      ...
      assert.equal(result.apiCallsPerMinute, 5);
      assert.equal(result.historyMaximumYears, 2);
...
console.log('verify-massive-stocks-basic-prerequisite-v1.test.js: PASS');
