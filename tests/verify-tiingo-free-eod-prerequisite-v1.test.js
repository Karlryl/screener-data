...
const script = path.join(root, 'scripts', 'verify-tiingo-free-eod-prerequisite-v1.py');
const contract = path.join(root, 'research', 'early-detection-v4', 'tiingo-free-eod-prerequisite-contract-v1.json');
const EXPECTED_RAW_SHA256 = '6ef3bbcce169fd840432c941ef83b31c6c30ee73194e955c1899d2c6530b888d';
...
for (const optimized of [false, true]) {
  ...
  for (const command of ['verify', 'self-test']) {
    ...
    if (command === 'self-test') {
      ...
      assert.equal(result.starterMonthlyFeeUsd, 0);
...
console.log('verify-tiingo-free-eod-prerequisite-v1.test.js: PASS');
