...
const ROOT=path.resolve(__dirname,'..'); const script=path.join(ROOT,'scripts','verify-quantconnect-free-cloud-pilot-v5.py');
...
const contract=JSON.parse(fs.readFileSync(path.join(ROOT,'research','early-detection-v4','quantconnect-free-cloud-pilot-contract-v5.json'),'utf8'));
...
const mutations=[
  ...
  x=>{x.rows[0].errors=[''];},
  x=>{x.rows[0].aliasResults[0].splitDates=['2010-01-04'];},
  x=>{x.rows[0].aliasResults[0].barCount=5844;},
  x=>{x.rows[0].errors=['CLOSE=123.45 adjustedClose=120'];},
  x=>{x.rows[0].aliasResults[0].errors=['CLOSE=123.45'];},
...
console.log('quantconnect-free-cloud-pilot-v5.test.js: PASS');
