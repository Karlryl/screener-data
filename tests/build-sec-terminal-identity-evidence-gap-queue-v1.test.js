...
const script = path.join(root, 'scripts', 'build-sec-terminal-identity-evidence-gap-queue-v1.py');
const contract = path.join(root, 'research', 'early-detection-v4', 'sec-terminal-identity-evidence-gap-queue-contract-v1.json');
const output = path.join(root, 'reports', 'early-detection', 'sec-terminal-identity-evidence-gap-queue-v1.json');
const EXPECTED_CONTRACT_RAW = 'afd71fca111888081723398603905bd296ee270b236e52e005721f7c83274954';
...
assert.doesNotMatch(source, /stockPrice|returnValue|favorableResult\s*[=:]/i);
console.log('build-sec-terminal-identity-evidence-gap-queue-v1.test.js: PASS');
