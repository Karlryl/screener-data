...
test('D5: gebundene Dateien und D1/D2/D4-Anker reproduzieren exakt', () => {
  ...
  assert.equal(abdeckung.ueberSiegel.length, 2, 'D5: Anzahl der ueber das aktuelle Siegel gepinnten Dateien');
  assert.equal(abdeckung.historisch.length, 4, 'D5: Anzahl der auf den historischen Hash gebundenen Dateien');
  assert.equal(abdeckung.waisen.length, 2, 'D5: Siegel-Schluessel ohne gebundene Datei - bis 30.08. ungeprueft, jetzt mitgeprueft');
