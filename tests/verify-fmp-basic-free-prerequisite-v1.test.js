...
const script = path.join(root, 'scripts', 'verify-fmp-basic-free-prerequisite-v1.py');
const contract = path.join(root, 'research', 'early-detection-v4', 'fmp-basic-free-prerequisite-contract-v1.json');
const EXPECTED_RAW_SHA256 = '9e7b0077c763875e01f65cb5e00a14d517d7ee75a884011404300288bf5796d8';
...
for (const optimized of [false, true]) {
  ...
  for (const command of ['verify', 'self-test']) {
    ...
    if (command === 'self-test') {
      ...
      assert.equal(result.historicalYears, 5);
      assert.equal(result.accountCreationAuthorizedNow, false);
...
console.log('verify-fmp-basic-free-prerequisite-v1.test.js: PASS');
