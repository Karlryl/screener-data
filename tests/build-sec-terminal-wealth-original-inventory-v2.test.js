...
const script = path.join(root, 'scripts', 'build-sec-terminal-wealth-original-inventory-v2.py');
...
for (const optimized of [false, true]) {
  ...
  assert.equal(result.ambiguousStatusRefAndSelectionMutationsRejected, true);
  assert.equal(result.malformedOrNonHeaderAccessionRejected, true);
  assert.equal(result.exactHeaderAccessionAccepted, true);
  assert.equal(result.outcomesAccessed, false);
}

console.log('build-sec-terminal-wealth-original-inventory-v2.test.js: PASS');
