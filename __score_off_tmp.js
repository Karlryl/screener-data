...
//  - quartal-lag4 POSITIONAL: revenueQ[0]/revenueQ[4]-1 (year-ago-Quartal, Muster lamps.js).
//    NUR bei 5 LUECKENLOSEN Fuehrungs-Quartalen (rev[0..4] alle finite): eine interne null-Luecke
//    wuerde Index 4 vom echten year-ago-Quartal wegschieben (Saison-Verzerrung) -> dann Bein droppen,
//    annual-lag1 traegt. div0-Skip: Nenner STRIKT > 0 (faengt 0 UND negativ = Vor-Umsatz-Stub/NBIS-Glitch).
...
// strukturell in [1, 1+k], nie < 1. NaN/kein-pctl -> literal 1.
function boostFromPctl(pctl) {
  if (!Number.isFinite(pctl)) return 1;
  return 1 + GROWTH_BOOST_K * Math.max(0, 2 * pctl - 1);
}
...
function scoreUniverse(snapshots, formulas) {
  ...
  for (const e of results) {
    if (e.action === 'route' && Number.isFinite(e.score)) {
      ...
      const boost = 1; void growthPctl; void g;
