...
function _convertSnapshotToUSD(snap) {
  ...
  if (snap.timeseries) {
    for (const key of Object.keys(snap.timeseries)) {
      ...
      // Währungsbeträge — gar nicht erst durch scale() jagen.
      // Nachgemessen 02.08.2026: scale() reicht Strings unverändert durch (Zeile 667:
      // typeof item !== 'object' -> return item), die Enden würden also auch OHNE dieses
      // continue heil bleiben. Der Skip ist die zweite Verteidigungslinie und bleibt —
...
function mapYahooToCanonical(yahoo, watchlistEntry, asOf) {
  ...
  // _arr trimmt nachlaufende Nullen JE FELD (Review 05.09., HIGH): fehlt COGS nur im aeltesten
  // Jahr, waere die Reihe kuerzer als annualRev und die Ableitung liefe fuer den GANZEN Ticker
  // ins Leere. Deshalb an annualRev ausrichten — dieselben isHist-Zeilen, newest-first, also nur
  // am Ende auffuellen bzw. kappen; die Indizes bleiben Jahr fuer Jahr identisch.
  while (annualCostOfRevenue.length < annualRev.length) annualCostOfRevenue.push(null);
  annualCostOfRevenue.length = annualRev.length;
  ...
  // NRE-SC-001 Teil 1: dieselbe Symmetrie für opIncQ — die dritte Quartalsreihe hatte als
  // einzige kein Perioden-Ende. revenueQEnds ersatzweise zu nehmen trägt NICHT: _arr trimmt
  // jede Reihe einzeln auf ihre letzte Nicht-Null, die Längen laufen also auseinander (eine
  // Firma kann ein operatives Ergebnis für ein Quartal führen, für das kein Umsatz gemeldet
  // ist). Reines Substrat, KEIN Guard — der Guard würde Scores ändern.
  const opIncQEnds = isHistQ.slice(0, opIncQ.length).map(r => _isoDay(_y(r, 'endDate')));
...
function mapFTSToQuarterly(quarterlyRows) {
  ...
  const opIncQEnds = []; // NRE-SC-001 Teil 1: dieselbe Row-Periode wie opIncQ
  ...
  for (const r of sorted) {
    ...
    opIncQEnds.push(_end); // NRE-SC-001: ebenfalls dieselbe FTS-Row
  }
  ...
  async function processOne(stock) {
    ...
    try {
      ...
      let _ftsAnchorDiverges = false;
      {
        ...
        // fremde Reihe moeglich — sichtbar machen, statt "0 abgeleitet" mit "nichts zu tun" zu verwechseln.
        if (canonical.annual.annualCostOfRevenue.length !== canonical.annual.annualRev.length) {
          _gpDerivedSkipped++;
          _log('INFO', `  ${stock.ticker}: GP-Ableitung uebersprungen — COGS-Reihe (${canonical.annual.annualCostOfRevenue.length}) und Umsatz-Reihe (${canonical.annual.annualRev.length}) ungleich lang`);
        }
        if (canonical.meta) {
          if (_gpAbl.derived > 0) {
