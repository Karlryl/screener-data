...
pre-cutoff SEC revenue/cash-flow trigger is associated with later *verifiable
...
- Only facts whose own filing reports the selected fiscal period may enter that
  period. Later comparative restatements do not revise the pre-cutoff series.
...
- Every eligible CIK is included. There is no outcome-informed sample,
  replacement, ticker repair, or post-cutoff eligibility decision.
...
accepted after the common observation cutoff.
...
positive anchors, fewer than 80 trigger-negative anchors, less than 90 percent
four-quarter ascertainment in either exposure group, or an ascertainment-rate
difference above 10 percentage points. SIC summaries are descriptive only.

## Pre-outcome barrier

Before any post-cutoff fact is read or materialized, a remote checkpoint must
...
The exposure pass must reject or quarantine every post-cutoff fact. The endpoint
