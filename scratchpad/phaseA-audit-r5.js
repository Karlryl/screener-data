export const meta = {
  name: 'phaseA-audit-r5',
  description: 'Phase-A Runde-5 Konvergenz-Audit: Regress ALLER R3+R4-Fixes + Restlücken, getaktet, 3-Stimmen-adversarial-Verify, Synthese',
  phases: [
...
const FIX_CONTEXT = `
DU AUDITIERST den hypergrowth-Scoring-Screener (Node.js) auf Branch loop/hypergrowth-neu @ 3e65b9dfd6.
...
DIES IST RUNDE 5 (Konvergenz-Check). Runde 3 fand 7 SA-Befunde -> Court entschied 5 Fälle (C1-C5), alle
umgesetzt. Runde 4 bestätigte die 5 Court-Fixes als SAUBER, fand 2 SA-Punkte -> beide gefixt:
- R4-Fix A (run-screener.js:101): Self-Baseline-Write jetzt ATOMAR (writeJsonAtomic statt fs.writeFileSync)
  -> der Torn-Write/baseline-wipe-Regress ist geschlossen. Prüfe: ist er WIRKLICH zu (atomar tmp+rename)?
- R4-Fix B (score.js trackOf OpInc-Zweig): NetIncome-Rescue feuert jetzt AUCH bei firstPresent(OpInc)===0
  (Lead-0-Stub 601162.SS -> financials unprofitable). + C4 wcov===1 Exact-Skip (8/8-Score exakt unverändert).
  Prüfe: kein anderer Name fälschlich geflippt? Anker unberührt?
RUNDE 4 stufte als NICHT-scoringAffecting ein (NICHT erneut als SA hochstufen ohne STARKE neue Evidenz):
gpGrowth gpYoY-Level (q rang-neutral), capEff cycleDiscount roic<0-Vorzeichen (1 Name ENEL.MI, kein Rang-
Flip, Fall-4-Territorium), calibrate.js (Offline), Display-Caps topN, A/H-Dedup cross-ccy (Deploy/kein US-
Primary). Wenn du DOCH starke Score/Rang-Evidenz findest -> melden, sonst als bekannt-non-SA bestätigen.

Prüfe ALLE Fixes (C1-C5 + peakMargin + R4-A/B) SCHARF auf REGRESS + verbleibende Lücken:
...
Synthese-Richter Runde 5 (Konvergenz-Check). Unten die BESTÄTIGTEN Befunde (>=2/3 Skeptiker konnten nicht
