...
const FIX_CONTEXT = `
DU AUDITIERST den hypergrowth-Scoring-Screener (Node.js) auf Branch loop/hypergrowth-neu @ 76d34d7863.
...
DIES IST RUNDE 8 (Konvergenz-Check, NEUES 7-ACHSEN-SCORING). WICHTIG: Karl hat per Direktive die Achse
revisionsMomentum (Analysten-Schätzungs-Revisionen) aus ALLEN 12 Formeln entfernt (8->7 Achsen; der
Screener bewertet auf harten Fundamentaldaten, kein Wall-Street-Sentiment; die Achse hatte nur 47%
Abdeckung). weightedScore renormiert die verbleibenden 7 Gewichte automatisch. PRÜFE DAS SCHARF:
- Renormieren die Gewichte in ALLEN 12 Sektoren sauber (kein NaN, kein Skalen-Bruch)? Besonders MATERIALS
  (revisionsMomentum hatte dort das SCHWERSTE Gewicht 2.4!) + financials/it-services (1.2) — sind die Scores
  nach dem Reweighting noch plausibel, oder dominiert jetzt eine Achse unplausibel?
- Wird ein Name durch die Reweighting unplausibel top/bottom? no-axes 3->4 (1 Name routete nur auf Analysten).
- Interagiert die Entfernung mit C4 (wcov = present-Gewicht/Gesamt-Gewicht ueber jetzt 7 Achsen)?
- Effekt-Baseline: CRDO 77.0->80.66 (Analysten-Achse war dessen 7.-Perzentil-Bremse). revisionsMomentum-
  Funktion bleibt in axes.js (Rechner, reversibel), ist aber in KEINE Formel mehr verdrahtet — kein Aufruf.
Verlauf: R3 fand 7 SA -> Court R3 (C1-C5) umgesetzt; R4 sauber + 2 SA gefixt; R5 + 1 SA -> Court R5; R6
KEINE Regression + 1 SA -> Court R6 (legRamp) gefixt. Alle 14 Fixes bleiben gueltig (achsen-unabhaengig):
- R6-Fix (lamps.js newestQtrSuspect): 6. Leg legRamp, rein ENTSCHAERFEND (&& !legRamp). LQDA (echte
  monotone Umsatz-Rampe 3M->133M, alle 4 QoQ-Schritte >1.35) triggerte leg4 faelschlich -> war data-suspect.
  legRamp de-firet, wenn ALLE 4 trailing QoQ-Umsatz-Schritte > REV_JUMP_MULT (1.35, aus leg3 wiederverwendet,
  KEINE neue Konstante). node: de-firet GENAU {LQDA}, Samsung/SNDK/ONEX feuern weiter. Prüfe: kann legRamp
  eine ECHTE Fabrikation exonerieren (Einzel-Q-Spike mit zufaellig 4 grossen Umsatz-Schritten)? Precision-safe?
- R5-Fix A (score.js Winsor): opMargin obere Schranke = oekon. Deckel OPMARGIN_CAP=1.0 (NICHT 'physisch' —
  REITs VICI/RPRX legit >1, ~12/14845, minimal geklemmt), untere + qoq data-learned. Prüfe: klemmt 1.0
  Legitimes rang-relevant? >1-Stubs korrekt? R5-Fix B hasCurrentRevenue strikt annualRev[0] (RTEZ raus).
- R4-Fix A (run-screener.js) atomarer Self-Baseline-Write. R4-Fix B trackOf NetIncome-Rescue bei OpInc[0]===0.
RUNDE 4/5/6 stuften als NICHT-scoringAffecting ein (NICHT erneut als SA hochstufen ohne STARKE neue Evidenz):
gpGrowth gpYoY-Level (q rang-neutral), capEff cycleDiscount roic<0-Vorzeichen (ENEL.MI 1 Name, Fall-4),
unterer opMargin-Floor (intendiert), calibrate.js (Offline), Display-Caps topN, dilution-Clamp,
A/H-Dedup + annualCurrencyLeak-Naming (Deploy/kein US-Primary).
...
BASELINE (HEAD 76d34d7863, 7-Achsen): route 3354, survival 73, data-suspect 16, no-axes 4, score≥90: ~1,
CRDO 80.66/PLTR 79.43/ALAB 70.4/BE 60.01/2383.TW 88.08/2308.TW 87.86/WTC.AX 68.98, 213 Tests grün, 0 NaN.
Firing-Set newestQtrSuspect = {005930.KS, ONEX.TO, SNDK}.
...
const FINDERS = [
  { key: 'revmom-removal-reweight', area: 'NEU (Karl-Direktive): revisionsMomentum aus allen 12 Formeln entfernt (8->7 Achsen). Prüfe SCHARF gegen echte Daten: (a) renormiert weightedScore in JEDEM der 12 Sektoren sauber (kein NaN/Skalen-Bruch, Score bleibt 0-100)? (b) MATERIALS hatte revisionsMomentum-Gewicht 2.4 (schwerste Achse!) — dominiert nach Entfernung eine verbleibende Achse unplausibel, ranken jetzt falsche Materials-Namen oben? financials/it-services (1.2) analog. (c) Wird ein Name durch das Reweighting unplausibel top/bottom (score/rank-flip an der Spitze)? (d) no-axes 3->4: welcher Name fiel raus, korrekt? (e) Interagiert mit C4-wcov (jetzt 7-Achsen-Gesamtgewicht) — bekommen Namen ohne die frühere Analysten-Achse jetzt anderes wcov? (f) ist die revisionsMomentum-Funktion in axes.js wirklich TOT (kein rawAxisValue-Aufruf)? node: scoreUniverse vor/nach vergleichen, größte Rang-Verschiebungen prüfen.' },
...
Synthese-Richter Runde 8 (Konvergenz-Check). Unten die BESTÄTIGTEN Befunde (>=2/3 Skeptiker konnten nicht
