...
function renderCard(ev, modeId, opts) {
  ...
  return `<div class="card" data-stock="${escHtml(JSON.stringify(stockSlim))}" data-af-url="${escHtml(afUrl)}" data-prof-state="${escHtml(profState)}" data-mcap="${Math.round(mcap||0)}" data-ipo="${ipoYear||0}" data-sector="${escHtml(sector)}" data-fcf-margin="${fcfMargin.toFixed(1)}" data-rev-growth="${revGrowth.toFixed(1)}" data-tier="${escHtml(tier)}" data-name="${escHtml(name.toLowerCase())}" data-ticker="${escHtml(ticker.toLowerCase())}">
  <div class="card-top">
...
(function() {
  ...
  function applyFilters(mode) {
    ...
    var secEl       = sb.querySelector('.sb-select');
    ...
    document.querySelectorAll('.mode-layout[data-mode="' + mode + '"] .card').forEach(function(card) {
      ...
      var show = psOk && mcapOk && ipoOk && secOk && fcfOk && growthOk && searchOk;
  ...
  document.addEventListener('click', function(e) {
    ...
    if (t.classList && t.classList.contains('reset-btn')) {
      ...
      var sec = sb.querySelector('.sb-select');
